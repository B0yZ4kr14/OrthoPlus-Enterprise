...
# Instalar Docker Engine + Docker Compose V2 (Layer 2 stack requirement)
log_info "Instalando Docker Engine + Docker Compose V2..."
...
log_success "Docker $(docker --version | awk '{print $3}' | tr -d ',') instalado"
log_success "Docker Compose $(docker compose version --short) instalado"
...
# Instalar Redis 7
log_info "Instalando Redis 7..."
apt install -y redis-server
# Configure Redis: bind to localhost only, set max memory, enable persistence
# Handle both commented and uncommented directive forms
REDIS_CONF="/etc/redis/redis.conf"
# maxmemory: replace existing setting (commented or not) or append
if grep -qE '^#?\s*maxmemory\s' "$REDIS_CONF"; then
    sed -i -E 's/^#?\s*maxmemory\s.*/maxmemory 256mb/' "$REDIS_CONF"
else
    echo "maxmemory 256mb" >> "$REDIS_CONF"
fi
# maxmemory-policy
if grep -qE '^#?\s*maxmemory-policy\s' "$REDIS_CONF"; then
    sed -i -E 's/^#?\s*maxmemory-policy\s.*/maxmemory-policy allkeys-lru/' "$REDIS_CONF"
else
    echo "maxmemory-policy allkeys-lru" >> "$REDIS_CONF"
fi
# appendonly
if grep -qE '^#?\s*appendonly\s' "$REDIS_CONF"; then
    sed -i -E 's/^#?\s*appendonly\s.*/appendonly yes/' "$REDIS_CONF"
else
    echo "appendonly yes" >> "$REDIS_CONF"
fi
# Restrict Redis to localhost — only replace the active (non-commented) bind line
if grep -qE '^bind\s' "$REDIS_CONF"; then
    sed -i -E 's/^bind\s.*/bind 127.0.0.1/' "$REDIS_CONF"
else
    echo "bind 127.0.0.1" >> "$REDIS_CONF"
fi
# Verify bind is set to localhost
if ! grep -qE '^bind 127\.0\.0\.1' "$REDIS_CONF"; then
    log_error "Redis bind configuration could not be set to 127.0.0.1 — please review ${REDIS_CONF}"
    exit 1
fi
systemctl enable redis-server
systemctl restart redis-server
log_success "Redis $(redis-server --version | awk '{print $3}' | cut -d= -f2) instalado e rodando em localhost:6379"
...
echo "1. Configure variáveis adicionais em .env (${INSTALL_DIR}/.env)"
