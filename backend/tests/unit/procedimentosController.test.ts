...
const procedimentoTemplates = (prisma as any).procedimento_templates as Record<string, jest.Mock>;
...
describe("ProcedimentosController", () => {
  ...
  describe("listTemplates", () => {
    ...
    it("filters by especialidade", async () => {
      const templates = [{ id: "t2", nome: "Extração", especialidade: "Cirurgia" }];
      procedimentoTemplates.findMany.mockResolvedValueOnce(templates);
      const req: Partial<Request> = { user: { clinicId: "clinic-1" } as any, body: {}, query: { especialidade: "Cirurgia" }, params: {} };
      const res = mockRes();
      await controller.listTemplates(req as Request, res);
      expect(procedimentoTemplates.findMany).toHaveBeenCalledWith({
  ...
  describe("getTemplateById", () => {
    it("returns 401 when clinicId is missing", async () => {
      const req: Partial<Request> = { user: undefined, body: {}, query: {}, params: { id: "t-x" } };
      const res = mockRes();
    ...
    it("returns 404 when template not found", async () => {
      ...
      const req: Partial<Request> = { user: { clinicId: "clinic-1" } as any, body: {}, query: {}, params: { id: "t-x" } };
      const res = mockRes();
      await controller.getTemplateById(req as Request, res);
  ...
  describe("updateTemplate", () => {
    it("returns 401 when clinicId is missing", async () => {
      const req: Partial<Request> = { user: undefined, body: {}, query: {}, params: { id: "t1" } };
      const res = mockRes();
    ...
    it("returns 400 on invalid input", async () => {
      ...
      const req: Partial<Request> = { user: { clinicId: "clinic-1" } as any, body: { nome: "" }, query: {}, params: { id: "t1" } };
      const res = mockRes();
      await controller.updateTemplate(req as Request, res);
      expect(res.status).toHaveBeenCalledWith(400);
    });
    ...
    it("updates and returns template", async () => {
      procedimentoTemplates.findFirst.mockResolvedValueOnce({ id: "t1", nome: "Old" });
  ...
  describe("deleteTemplate", () => {
    ...
    it("deletes and returns 204", async () => {
      ...
      const res = mockRes();
      await controller.deleteTemplate(req as Request, res);
      expect(procedimentoTemplates.deleteMany).toHaveBeenCalledWith({ where: { id: "t1", clinic_id: "clinic-1" } });
