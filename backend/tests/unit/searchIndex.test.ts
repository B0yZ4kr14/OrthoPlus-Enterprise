...
import { BaseIndexer, SearchIndexEntry } from '../../src/modules/search_index/services/BaseIndexer'
...
class TestIndexer extends BaseIndexer<{ id: string; name: string }> {
  ...
  protected async queryBatch(cursor?: string, since?: Date): Promise<{ id: string; name: string }[]> {
  ...
  protected buildIndexEntry(entity: { id: string; name: string }): SearchIndexEntry {
    return {
...
describe('PacienteIndexer', () => {
  ...
  describe('toEntry', () => {
    it('should correctly concatenate searchable fields into content', async () => {
      ...
      const createManyCall = (prisma.search_index.createMany as jest.Mock).mock.calls[0][0]
...
describe('SearchIndexPatientHandler', () => {
  ...
  it('should catch errors and not throw', async () => {
    ;(prisma.patients.findUnique as jest.Mock).mockRejectedValue(new Error('DB error'))
