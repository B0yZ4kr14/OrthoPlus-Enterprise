...
describe("searchCache", () => {
  ...
  describe("getSearchCache", () => {
    it("should return parsed cached data on hit", async () => {
      ...
      ;(redisInstance.get as jest.Mock).mockResolvedValue(JSON.stringify(mockData))

      const result = await getSearchCache("clinic-1", "joao", "pacientes", 1, 20)
    ...
    it("should return null and not throw on Redis error", async () => {
      ;(redisInstance.get as jest.Mock).mockRejectedValue(new Error("Redis down"))
  ...
  describe("setSearchCache", () => {
    ...
    it("should not throw on Redis error", async () => {
      ;(redisInstance.set as jest.Mock).mockRejectedValue(new Error("Redis down"))
  ...
  describe("invalidateSearchCache", () => {
    it("should delete all keys matching search:clinicId:*", async () => {
      ;(redisInstance.scan as jest.Mock)
        .mockResolvedValueOnce(["0", ["search:clinic-1:joao:__all__:1:20"]])
    ...
    it("should not throw on Redis error", async () => {
      ;(redisInstance.scan as jest.Mock).mockRejectedValue(new Error("Redis down"))
