...
describe("ContradictionDetector", () => {
    ...
    const repo = createMockRepo([
      {
        ...
        frontmatter: JSON.stringify({
          rawContent:
            "- **MEM-FR-001**: The system MUST index all documents.\n- **MEM-FR-002**: The system MUST provide search.",
        }),
      },
      {
        ...
        frontmatter: JSON.stringify({
          rawContent:
            "- **OTH-FR-001**: The system MUST send emails.\n- **OTH-FR-002**: The system MUST log events.",
        }),
      },
    ]);
  ...
  it("should detect semantic contradiction on same topic with opposite modality", () => {
    const repo = createMockRepo([
      {
        ...
        frontmatter: JSON.stringify({
          rawContent:
            "- **MEM-FR-010**: The system MUST expose a CLI interface.",
        }),
      },
      {
        sourcePath: "specs/002-conflict/spec.md",
        ...
        frontmatter: JSON.stringify({
          rawContent:
            "- **MEM-FR-020**: The system MUST NOT expose a CLI interface.",
        }),
      },
    ]);
    ...
    const contradiction = result.find(
      (r) => r.type === "contradictory_requirement" && r.severity === "high",
    );
    ...
    const repo = createMockRepo([
      {
        ...
        frontmatter: JSON.stringify({
          rawContent:
            "- **AUTH-FR-001**: The system MUST validate API keys on startup.",
        }),
      },
      {
        ...
        frontmatter: JSON.stringify({
          rawContent:
            "- **SEC-FR-001**: The system MUST validate API keys on every request.",
        }),
      },
    ]);
