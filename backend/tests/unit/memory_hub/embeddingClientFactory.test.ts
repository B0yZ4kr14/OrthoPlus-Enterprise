...
describe("EmbeddingClientFactory.updateConfig", () => {
  ...
  it("should update provider", () => {
    const t = makeDummy(64)
    const config = EmbeddingClientFactory.updateConfig({ provider: "google", apiKey: t })
  ...
  it("should reject invalid API key", () => {
    expect(() => EmbeddingClientFactory.updateConfig({ apiKey: "short" })).toThrow("too short")
  })

  it("should allow ollama without API key", () => {
    const config = EmbeddingClientFactory.updateConfig({ provider: "ollama", apiKey: undefined })
