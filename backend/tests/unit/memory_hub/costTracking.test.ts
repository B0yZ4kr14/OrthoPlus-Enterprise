...
describe("CostTrackingService", () => {
  ...
  it("estimates cost for openai small", () => {
    const estimate = service.estimateCost(1_000_000, "openai", "text-embedding-3-small")
    expect(estimate.costUsd).toBe(0.02)
    expect(estimate.tokens).toBe(1_000_000)
  })

  it("estimates zero cost for ollama", () => {
    const estimate = service.estimateCost(1_000_000, "ollama", "nomic-embed-text")
    expect(estimate.costUsd).toBe(0)
  })

  it("logs cost and returns estimate", () => {
    const result = service.logCost("clinic-1", "test query", "openai", "text-embedding-3-small")
  ...
  it("triggers alert when budget is exceeded", () => {
    ...
    service.logCost("clinic-2", "a".repeat(4000), "openai", "text-embedding-3-small")
  ...
  it("aggregates costs per month correctly", () => {
    service.logCost("clinic-3", "query one", "openai", "text-embedding-3-small")
    service.logCost("clinic-3", "query two", "openai", "text-embedding-3-small")
