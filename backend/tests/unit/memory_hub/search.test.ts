...
describe("SearchService", () => {
  ...
  describe("T014: semantic search returns results ordered by relevance", () => {
    ...
    it("deduplicates by document keeping highest scoring chunk", async () => {
      ...
      const doc1Result = result.results.find((r) => r.sourcePath.includes("001-pacientes"))
    ...
    it("infers docType from sourcePath", async () => {
      ...
      expect(result.results.find((r) => r.sourcePath.includes("specs/"))?.docType).toBe("spec")
      expect(result.results.find((r) => r.sourcePath.includes("plans/"))?.docType).toBe("plan")
      expect(result.results.find((r) => r.sourcePath.includes(".specify/memory/"))?.docType).toBe("memory")
      expect(result.results.find((r) => r.sourcePath.includes("docs/"))?.docType).toBe("doc")
    })
  ...
  describe("T015: search filters by doc_type and archived status", () => {
    it("passes docTypes filter to EmbeddingRepository", async () => {
      ...
      await searchService.search("rate limiting", { docTypes: ["spec", "plan"] })
    ...
    it("filters by single docType", async () => {
      ...
      const result = await searchService.search("patient", { docTypes: ["spec"] })
