import { PIIDetector, piiDetector } from "../../../src/modules/memory_hub/infrastructure/PIIDetector"
...
describe("PIIDetector", () => {
  describe("scan", () => {
    it("should detect CPF in content", () => {
      ...
      expect(result.detectedTypes).toContainEqual(expect.stringContaining("CPF"))
    ...
    it("should detect email addresses", () => {
      ...
      const result = piiDetector.scan(content)
      expect(result.hasPII).toBe(true)
      expect(result.detectedTypes).toContainEqual(expect.stringContaining("EMAIL"))
    })
    ...
    it("should detect Brazilian phone numbers", () => {
      ...
      const result = piiDetector.scan(content)
      expect(result.hasPII).toBe(true)
      expect(result.detectedTypes).toContainEqual(expect.stringContaining("PHONE_BR"))
    })
    ...
    it("should detect credit card numbers", () => {
      ...
      const result = piiDetector.scan(content)
      expect(result.hasPII).toBe(true)
      expect(result.detectedTypes).toContainEqual(expect.stringContaining("CREDIT_CARD"))
  ...
  describe("hasLawfulBasis", () => {
    it("should return true for lawful_basis frontmatter", () => {
      expect(piiDetector.hasLawfulBasis({ lawful_basis: "legitimate_interest" })).toBe(true)
    })
    ...
    it("should return true for legal_basis frontmatter", () => {
      expect(piiDetector.hasLawfulBasis({ legal_basis: "contract" })).toBe(true)
    })
