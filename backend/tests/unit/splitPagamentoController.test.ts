...
const splitConfig = (prisma as any).split_payment_config as Record<
  string,
  jest.Mock
>;
const splitComissoes = (prisma as any).split_comissoes as Record<
  string,
  jest.Mock
>;
const splitTransactions = (prisma as any).split_transactions as Record<
  string,
  jest.Mock
>;
...
describe("SplitPagamentoController.getConfig", () => {
  ...
  it("returns 500 on database error", async () => {
    ...
    const req = mockReq();
    const res = mockRes();
    await expect(controller.getConfig(req as Request, res)).rejects.toThrow(
      "DB",
    );
  });
});
...
describe("SplitPagamentoController.upsertConfig", () => {
  ...
  it("creates config when none exists", async () => {
    ...
    splitConfig.create.mockResolvedValueOnce({
      ...sampleConfig,
      percentage: 30,
    });
  ...
  it("updates config when one already exists", async () => {
    ...
    splitConfig.update.mockResolvedValueOnce({
      ...sampleConfig,
      percentage: 30,
    });
    const req = mockReq({ body: validBody });
  ...
  it("returns 500 on database error", async () => {
    ...
    const req = mockReq({ body: validBody });
    const res = mockRes();
    await expect(controller.upsertConfig(req as Request, res)).rejects.toThrow(
      "DB",
    );
  });
});
...
describe("SplitPagamentoController.calculateSplit", () => {
  ...
  it("calculates split amounts correctly", async () => {
    ...
    splitConfig.findFirst.mockResolvedValueOnce({
      ...sampleConfig,
      percentage: 40,
    });
  ...
  it("falls back to config without procedure_type when not found with it", async () => {
    ...
    const req = mockReq({
      body: { ...validBody, procedure_type: "ORTODONTIA" },
    });
    const res = mockRes();
    await controller.calculateSplit(req as Request, res);
