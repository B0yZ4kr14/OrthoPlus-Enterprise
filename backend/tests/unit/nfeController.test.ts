...
jest.mock(
  "../../src/modules/nfe/infrastructure/repositories/NFeRepositoryPostgres",
  () => {
    return {
...
beforeAll(() => {
  mockRepo = (NFeRepositoryPostgres as unknown as jest.Mock).mock.results[0]
    .value as MockRepo;
});
...
describe("NFeController.list", () => {
  ...
  it("calls repository.findAll and returns result", async () => {
    ...
    const req = mockReq({
      query: { status: "RASCUNHO", take: "10", skip: "0" },
    });
    const res = mockRes();
    await controller.list(req as Request, res);
...
describe("NFeController.create", () => {
  ...
  it("propagates repository error", async () => {
    ...
    const req = mockReq({ body: validBody });
    const res = mockRes();
    await expect(controller.create(req as Request, res)).rejects.toThrow(
      "DB error",
    );
  });
});
...
describe("NFeController.cancel", () => {
  ...
  it("returns 422 when domain cancelar throws (e.g. not AUTORIZADA)", async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({
      error: "Apenas NFe autorizada pode ser cancelada",
    });
  });
