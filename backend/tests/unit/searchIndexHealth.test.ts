...
describe("SearchIndexController.health", () => {
  ...
  it("should return healthy when all checks pass", async () => {
    ;(prisma.$queryRaw as jest.Mock)
      ...
      .mockResolvedValueOnce([{ max_updated: new Date("2026-05-26T10:00:00Z") }])
    ;(redisInstance.ping as jest.Mock).mockResolvedValue("PONG")

    await (controller as any).health(req as Request, res as Response, jest.fn())
    ...
    expect(jsonMock).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        checks: expect.objectContaining({
          postgresql: expect.objectContaining({ status: "ok", details: "10 indexed records" }),
  ...
  it("should return degraded when PostgreSQL fails", async () => {
    ;(prisma.$queryRaw as jest.Mock).mockRejectedValue(new Error("PG connection refused"))
    ;(redisInstance.ping as jest.Mock).mockResolvedValue("PONG")
  ...
  it("should return degraded when Redis fails", async () => {
    ...
    ;(redisInstance.ping as jest.Mock).mockRejectedValue(new Error("Redis down"))
