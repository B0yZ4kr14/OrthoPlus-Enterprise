...
const mockReq = (
  ...
  user:
    user !== undefined
      ? user
      : { id: "user-1", clinicId: "clinic-1", role: "ADMIN" },
});
...
const mockCadastrarUseCase = {
  execute: jest.fn().mockResolvedValue({ patientId: "pid-1" }),
};
const mockAtualizarUseCase = {
  execute: jest.fn().mockResolvedValue({ patientId: "pid-1" }),
};
const mockAlterarStatusUseCase = {
...
describe("PacientesController", () => {
  ...
  describe("create", () => {
    ...
    it("returns 500 when use-case throws (duplicate CPF)", async () => {
      mockCadastrarUseCase.execute.mockRejectedValueOnce(
        new Error("Já existe paciente cadastrado com este CPF"),
      );
      const req = mockReq({ fullName: "João Silva", cpf: "529.982.247-25" });
      const res = mockRes();
      await controller.create(req as Request, res, jest.fn());
  ...
  describe("update", () => {
    ...
    it("returns 500 when use-case throws", async () => {
      mockAtualizarUseCase.execute.mockRejectedValueOnce(
  ...
  describe("list", () => {
    ...
    it("returns 500 on repository error", async () => {
      mockPatientRepository.findMany.mockRejectedValueOnce(
        new Error("DB error"),
      );
      const req = mockReq({}, {}, {}, {});
      const res = mockRes();
  ...
  describe("statsByStatus", () => {
    it("returns counts", async () => {
      mockPatientRepository.countByStatus.mockResolvedValueOnce({
        PROSPECT: 5,
        TRATAMENTO: 2,
      });
      const req = mockReq();
  ...
  describe("getPatientTimeline", () => {
    it("returns timeline array", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith(
        expect.objectContaining({ timeline: [] }),
      );
    });
  ...
  describe("patientAuth", () => {
    ...
    it("returns 200 on logout", async () => {
      mockPatientRepository.deletePatientSessionsBySessionId.mockResolvedValueOnce(
        { count: 1 },
      );
...
describe("PatientQueryController", () => {
  ...
  describe("getById", () => {
    it("returns 200 when patient found", async () => {
      mockGetQuery.execute.mockResolvedValueOnce({
        id: "pid-1",
        fullName: "João",
      });
      const req = mockReq({}, {}, { id: "pid-1" });
  ...
  describe("list", () => {
    it("returns 200 with paginated results", async () => {
      mockListQuery.execute.mockResolvedValueOnce({
        items: [],
        total: 0,
        page: 1,
        totalPages: 0,
  ...
  describe("getStats", () => {
    it("returns 200 with stats", async () => {
      mockStatsQuery.execute.mockResolvedValueOnce({
        total: 10,
        ativos: 5,
        inativos: 5,
        arquivados: 0,
