...
describe("TISSController.listGuias", () => {
  it("returns 401 when no clinicId", async () => {
    ...
    const res = mockRes();
    await expect(
      controller.listGuias(req as Request, res),
  ...
  it("filters by insurance_company and status", async () => {
    ...
    const req = mockReq({
      query: { insurance_company: "Amil", status: "SUBMITTED" },
    });
    const res = mockRes();
    await controller.listGuias(req as Request, res);
    expect(guides.findMany).toHaveBeenCalledWith(
  ...
  it("returns 500 on database error", async () => {
    ...
    const req = mockReq();
    const res = mockRes();
    await expect(controller.listGuias(req as Request, res)).rejects.toThrow(
      "DB",
    );
  });
});
...
describe("TISSController.getGuiaById", () => {
  it("returns 401 when no clinicId", async () => {
    ...
    const res = mockRes();
    await expect(
      controller.getGuiaById(req as Request, res),
    ).rejects.toMatchObject({ status: 401 });
  });
  ...
  it("returns 500 on database error", async () => {
    ...
    const req = mockReq({ params: { id: "guide-1" } });
    const res = mockRes();
    await expect(controller.getGuiaById(req as Request, res)).rejects.toThrow(
      "DB",
    );
  });
});
...
describe("TISSController.createGuia", () => {
  ...
  it("returns 401 when no clinicId", async () => {
    ...
    const res = mockRes();
    await expect(
      controller.createGuia(req as Request, res),
    ).rejects.toMatchObject({ status: 401 });
  });
  ...
  it("creates a guide and returns 201", async () => {
    guides.create.mockResolvedValueOnce({
      ...sampleGuide,
      clinic_id: "clinic-1",
    });
    ...
    expect(guides.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ clinic_id: "clinic-1" }),
      }),
    );
  ...
  it("returns 500 on database error", async () => {
    ...
    const req = mockReq({ body: validBody });
    const res = mockRes();
    await expect(controller.createGuia(req as Request, res)).rejects.toThrow(
      "DB",
    );
  });
});
...
describe("TISSController.updateGuia", () => {
  it("returns 401 when no clinicId", async () => {
    ...
    const res = mockRes();
    await expect(
      controller.updateGuia(req as Request, res),
    ).rejects.toMatchObject({ status: 401 });
  });
...
describe("TISSController.deleteGuia", () => {
  it("returns 401 when no clinicId", async () => {
    ...
    const res = mockRes();
    await expect(
      controller.deleteGuia(req as Request, res),
    ).rejects.toMatchObject({ status: 401 });
  });
...
describe("TISSController.listLotes", () => {
  it("returns 401 when no clinicId", async () => {
    ...
    const res = mockRes();
    await expect(
      controller.listLotes(req as Request, res),
    ).rejects.toMatchObject({ status: 401 });
  });
  ...
  it("filters by status", async () => {
    ...
    expect(batches.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { clinic_id: "clinic-1", status: "SUBMITTED" },
      }),
    );
  });
});
...
describe("TISSController.createLote", () => {
  ...
  it("returns 401 when no clinicId", async () => {
    ...
    const res = mockRes();
    await expect(
      controller.createLote(req as Request, res),
    ).rejects.toMatchObject({ status: 401 });
  });
...
describe("TISSController.submitBatch", () => {
  ...
  it("returns 401 when no clinicId", async () => {
    ...
    const res = mockRes();
    await expect(
      controller.submitBatch(req as Request, res),
    ).rejects.toMatchObject({ status: 401 });
  });
  ...
  it("returns 500 on database error", async () => {
    ...
    const req = mockReq({ body: validBody });
    const res = mockRes();
    await expect(controller.submitBatch(req as Request, res)).rejects.toThrow(
      "DB",
    );
  });
});
...
describe("TISSController.getStatistics", () => {
  it("returns 401 when no clinicId", async () => {
    ...
    const res = mockRes();
    await expect(
      controller.getStatistics(req as Request, res),
    ).rejects.toMatchObject({ status: 401 });
  });

  it("returns aggregated statistics", async () => {
    guides.groupBy.mockResolvedValueOnce([
      { status: "SUBMITTED", _count: { id: 2 }, _sum: { amount: 400 } },
    ]);
    batches.groupBy.mockResolvedValueOnce([
      { status: "SUBMITTED", _count: { id: 1 }, _sum: { total_amount: 400 } },
    ]);
    guides.aggregate.mockResolvedValueOnce({
      _count: { id: 2 },
      _sum: { amount: 400 },
    });
    guides.aggregate.mockResolvedValueOnce({
  ...
  it("returns 500 on database error", async () => {
    ...
    const req = mockReq();
    const res = mockRes();
    await expect(controller.getStatistics(req as Request, res)).rejects.toThrow(
      "DB",
    );
  });
});
