...
describe("FidelidadeController", () => {
  ...
  describe("getPoints", () => {
    it("returns 401 when clinicId is missing", async () => {
      ...
      const res = mockRes();
      await expect(controller.getPoints(req as Request, res)).rejects.toThrow(
    ...
    it("returns 500 when database throws", async () => {
      (prisma as any).fidelidade_pontos.findMany.mockRejectedValueOnce(
        new Error("DB fail"),
      );
      const req: Partial<Request> = {
        user: { clinicId: "clinic-1" } as any,
  ...
  describe("addPoints", () => {
    it("returns 401 when clinicId is missing", async () => {
      ...
      const res = mockRes();
      await expect(controller.addPoints(req as Request, res)).rejects.toThrow(
        "Missing clinic context",
      );
    });
    ...
    it("returns 400 for invalid input", async () => {
      const req: Partial<Request> = {
        user: { clinicId: "clinic-1" } as any,
        body: { patient_id: "not-uuid", pontos: -5 },
        query: {},
        params: {},
      };
      const res = mockRes();
    ...
    it("creates points atomically and returns 201 with accumulated points", async () => {
      ...
      (prisma as any).fidelidade_pacientes.findFirst.mockResolvedValue({
        id: "fp1",
        patient_id: payload.patient_id,
        pontos_acumulados: 25,
      });
      (prisma as any).fidelidade_pacientes.update.mockResolvedValue({
        id: "fp1",
        patient_id: payload.patient_id,
        pontos_acumulados: 25,
      });
  ...
  describe("listBadges", () => {
    it("returns 401 when clinicId is missing", async () => {
      ...
      const res = mockRes();
      await expect(controller.listBadges(req as Request, res)).rejects.toThrow(
        "Missing clinic context",
      );
    });
    ...
    it("returns 500 on DB error", async () => {
      (prisma as any).fidelidade_badges.findMany.mockRejectedValueOnce(
        new Error("fail"),
      );
      const req: Partial<Request> = {
        user: { clinicId: "clinic-1" } as any,
  ...
  describe("createBadge", () => {
    it("returns 400 for invalid input", async () => {
      ...
      const res = mockRes();
      await expect(controller.createBadge(req as Request, res)).rejects.toThrow(
        "Invalid input",
      );
    });
  ...
  describe("listRecompensas", () => {
    it("returns 401 when clinicId is missing", async () => {
      ...
      const res = mockRes();
      await expect(
        controller.listRecompensas(req as Request, res),
      ).rejects.toThrow("Missing clinic context");
    });

    it("filters by ativo query param", async () => {
      const fake = [{ id: "r1", ativo: true }];
      (prisma as any).fidelidade_recompensas.findMany.mockResolvedValueOnce(
        fake,
      );
      const req: Partial<Request> = {
        user: { clinicId: "clinic-1" } as any,
      ...
      expect(
        (prisma as any).fidelidade_recompensas.findMany,
    ...
    it("returns 500 on DB error", async () => {
      (prisma as any).fidelidade_recompensas.findMany.mockRejectedValueOnce(
        new Error("fail"),
      );
      const req: Partial<Request> = {
        user: { clinicId: "clinic-1" } as any,
  ...
  describe("createRecompensa", () => {
    it("returns 400 for invalid input", async () => {
      ...
      const res = mockRes();
      await expect(
        controller.createRecompensa(req as Request, res),
    ...
    it("creates recompensa and returns 201", async () => {
      ...
      (prisma as any).fidelidade_recompensas.create.mockResolvedValueOnce(
        created,
      );
      const req: Partial<Request> = {
        user: { clinicId: "clinic-1" } as any,
  ...
  describe("listIndicacoes", () => {
    it("returns 401 when clinicId is missing", async () => {
      ...
      const res = mockRes();
      await expect(
        controller.listIndicacoes(req as Request, res),
      ).rejects.toThrow("Missing clinic context");
    });

    it("filters by referrer_id", async () => {
      const fake = [{ id: "i1", referrer_id: "ref-1" }];
      (prisma as any).fidelidade_indicacoes.findMany.mockResolvedValueOnce(
        fake,
      );
      const req: Partial<Request> = {
        user: { clinicId: "clinic-1" } as any,
        body: {},
        query: { referrer_id: "ref-1" },
        params: {},
      };
      const res = mockRes();
      await controller.listIndicacoes(req as Request, res);
      expect(
        (prisma as any).fidelidade_indicacoes.findMany,
      ).toHaveBeenCalledWith({
    ...
    it("returns 500 on DB error", async () => {
      (prisma as any).fidelidade_indicacoes.findMany.mockRejectedValueOnce(
        new Error("fail"),
      );
      const req: Partial<Request> = {
        user: { clinicId: "clinic-1" } as any,
  ...
  describe("createIndicacao", () => {
    it("returns 400 for invalid input", async () => {
      const req: Partial<Request> = {
        user: { clinicId: "clinic-1" } as any,
        body: { referrer_id: "bad-id" },
        query: {},
        params: {},
      };
      const res = mockRes();
      await expect(
        controller.createIndicacao(req as Request, res),
      ).rejects.toThrow("Invalid input");
    });
    ...
    it("creates indicacao and returns 201", async () => {
      ...
      (prisma as any).fidelidade_indicacoes.create.mockResolvedValueOnce(
        created,
      );
      const req: Partial<Request> = {
        user: { clinicId: "clinic-1" } as any,
    ...
    it("returns 500 when database throws", async () => {
      (prisma as any).fidelidade_indicacoes.create.mockRejectedValueOnce(
        new Error("DB fail"),
      );
      const req: Partial<Request> = {
        user: { clinicId: "clinic-1" } as any,
