...
describe("MemoryHub Offline Operation", () => {
  ...
  beforeEach(() => {
    ...
    const schemaSql = fs.readFileSync(
      path.join(__dirname, "../../../src/modules/memory_hub/infrastructure/initSchema.sql"),
      "utf-8",
    )
    db.exec(schemaSql)
  })
  ...
  it("should handle search with mocked embeddings", async () => {
    const mockEmbedder = new OllamaEmbeddingClient() as jest.Mocked<OllamaEmbeddingClient>
    mockEmbedder.embedSingle = jest.fn().mockResolvedValue({
      embedding: [0.1, 0.2, 0.3],
      model: "test-model",
    })

    const mockEmbeddings = new EmbeddingRepository(db) as jest.Mocked<EmbeddingRepository>
    mockEmbeddings.searchSimilar = jest.fn().mockReturnValue([])

    const mockDocuments = new DocumentRepository(db) as jest.Mocked<DocumentRepository>
    const searchService = new SearchService(mockEmbedder, mockEmbeddings, mockDocuments)
