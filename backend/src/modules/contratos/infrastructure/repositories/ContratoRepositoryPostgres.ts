...
export class ContratoRepositoryPostgres implements IContratoRepository {
  async findById(id: string): Promise<Contrato | null> {
    const result = await db.query(
      "SELECT * FROM contratos.contratos WHERE id = $1",
      [id],
    );
  ...
  async findAll(options: {
    ...
    const countQuery =
      "SELECT COUNT(*) FROM contratos.contratos WHERE " + conditions;
    const countResult = await db.query(countQuery, params);
    const total = parseInt(countResult.rows[0].count, 10);

    let query =
      "SELECT * FROM contratos.contratos WHERE " +
      conditions +
  ...
  private mapToEntity(row: Record<string, unknown>): Contrato {
    return Contrato.create({
      ...
      items:
        row.items as import("../../domain/entities/Contrato").ItemContrato[],
      ...
      status: row.status as
        ...
        | "CONCLUIDO"
        | "CANCELADO",
      formaPagamento: row.forma_pagamento as
        | "AVISTA"
        | "PARCELADO"
        | "MENSALIDADE",
