...
export class NFeRepositoryPostgres implements INFeRepository {
  ...
  private static readonly ALLOWED_LOOKUP_COLUMNS: ReadonlySet<string> = new Set(
    ["id", "numero"],
  );
  ...
  async findAll(
    options: FindAllNFeOptions,
  ): Promise<{ items: NFe[]; total: number }> {
    ...
    try {
      ...
      return {
        items: dataResult.rows.map((databaseRow) =>
          this.mapToEntity(databaseRow as NFeRow),
        ),
        total,
      };
    } catch (err: any) {
  ...
  async save(nfe: NFe): Promise<void> {
    try {
      ...
      if (err.code === "42P01") {
        ...
        throw new Error(
          "Módulo NF-e não está configurado — tabela fiscal.nfes não existe",
        );
      }
      throw err;
    }
