...
export class ModuloRepositoryPostgres implements IModuloRepository {
  ...
  async findAll(options: {
    ...
    categoria?: string;
    onlyActive?: boolean;
  }): Promise<{ items: Modulo[]; total: number }> {
    ...
    const countQuery =
      "SELECT COUNT(*) FROM configuracoes.modulos WHERE " + conditions;
    const countResult = await db.query(countQuery, params);
    const total = parseInt(countResult.rows[0].count, 10);

    const result = await db.query(
      "SELECT * FROM configuracoes.modulos WHERE " +
        conditions +
        " ORDER BY nome ASC",
      params,
    );
  ...
  async save(modulo: Modulo): Promise<void> {
    await db.query(
      ...
      [
        ...
        modulo.categoria,
        modulo.isActive,
        JSON.stringify(modulo.dependencies),
        JSON.stringify(modulo.configuracoes),
        modulo.createdAt,
        modulo.updatedAt,
      ],
    );
  ...
  private mapToEntity(row: Record<string, unknown>): Modulo {
    return Modulo.create({
      ...
      dependencies:
        row.dependencies as import("../../domain/entities/Modulo").ModuloDependency[],
