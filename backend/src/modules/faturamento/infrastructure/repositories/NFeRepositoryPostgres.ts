...
export class NFeRepositoryPostgres implements INFeRepository {
  async findById(id: string): Promise<NFe | null> {
    const result = await db.query(
      "SELECT * FROM faturamento.nfes WHERE id = $1",
      [id],
    );
  ...
  async findAll(
    ...
    const countQuery =
      "SELECT COUNT(*) FROM faturamento.nfes WHERE " + conditions;
    const dataQuery =
      "SELECT * FROM faturamento.nfes WHERE " +
      conditions +
