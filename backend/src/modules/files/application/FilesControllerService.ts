...
export class FilesControllerService {
  ...
  async uploadFile(params: {
    ...
    let visibilidade = sanitizeUploadVisibility(body.visibilidade as string | undefined, userRole)
    ...
    MetricsEmitter.incrementCounter("files_uploaded", "Files uploaded", { clinicId, categoria: fileRecord.categoria })
    ...
    }).catch((err) => logger.warn("[FilesService] Audit log failed (non-blocking):", err))

    return fileRecord
  }

  async listFiles(clinicId: string, userRole: "ADMIN" | "MEMBER" | "PATIENT" | undefined, query: any) {
  ...
  async getFile(id: string, clinicId: string, userRole: string | undefined, meta: { ip?: string; userAgent?: string; userId?: string }) {
  ...
  async downloadFile(id: string, clinicId: string, userRole: string | undefined, meta: { ip?: string; userAgent?: string; userId?: string }) {
    const startTime = Date.now()
    const file = await this.filesService.getById(id, clinicId)
    if (!file) throw Errors.notFound("File", id)
    enforceVisibilityAcl(file, userRole)
  ...
  async deleteFile(id: string, clinicId: string, meta: { ip?: string; userAgent?: string; userId?: string }) {
    const file = await this.filesService.getById(id, clinicId)
    if (!file) throw Errors.notFound("File", id)
    ...
    MetricsEmitter.incrementCounter("files_deleted", "Files deleted", { clinicId, categoria: file.categoria })
  ...
  async uploadBackupToCloud(backupId: string, provider: string, config: any) {
    ...
    await this.repo.updateBackup(backupId, {
      file_path: uploadUrl,
      metadata: {
        ...(typeof backup.metadata === "object" && backup.metadata !== null ? backup.metadata : {}),
  ...
  private async uploadToS3(data: string, fileName: string, config: S3Config): Promise<string> {
  ...
  private async uploadToGoogleDrive(data: string, fileName: string, config: GoogleDriveConfig): Promise<string> {
    ...
    if (!tokenResponse.ok) throw new Error("Failed to refresh Google Drive token")

    const { access_token } = (await tokenResponse.json()) as { access_token: string }

    const metadata = { name: fileName, parents: config.folderId ? [config.folderId] : [] }
  ...
  private async uploadToDropbox(data: string, fileName: string, config: DropboxConfig): Promise<string> {
    const dropboxPath = config.folder ? `/${config.folder}/${fileName}` : `/${fileName}`
  ...
  async createVersion(fileId: string, clinicId: string, userId: string, file: Express.Multer.File) {
