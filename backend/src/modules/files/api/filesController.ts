...
export class FilesController {
  uploadFile = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!clinicId || !userId) throw Errors.unauthorized("Authentication required")
  ...
  listFiles = asyncHandler(async (req: Request, res: Response) => {
    ...
    const files = await service.listFiles(clinicId, req.user?.role as any, req.query)
  ...
  getFile = asyncHandler(async (req: Request, res: Response) => {
    const clinicId = req.user?.clinicId as string
    if (!clinicId) throw Errors.unauthorized("Authentication required")
    const { file, existsOnDisk, storagePath } = await service.getFile(req.params.id, clinicId, req.user?.role as string | undefined, {
      ip: req.ip ?? undefined,
      userAgent: req.headers["user-agent"] as string | undefined,
      userId: req.user?.id as string | undefined,
    })
    res.status(200).json({ success: true, data: { ...file, storagePath, existsOnDisk } })
  })

  downloadFile = asyncHandler(async (req: Request, res: Response) => {
    const clinicId = req.user?.clinicId as string
    if (!clinicId) throw Errors.unauthorized("Authentication required")
    const { file, filePath } = await service.downloadFile(req.params.id, clinicId, req.user?.role as string | undefined, {
      ip: req.ip ?? undefined,
      userAgent: req.headers["user-agent"] as string | undefined,
      userId: req.user?.id as string | undefined,
    })
    res.setHeader("Content-Type", file.mimeType)
    res.setHeader("Content-Disposition", `attachment; filename="${encodeURIComponent(file.nomeOriginal)}"`)
  ...
  uploadBackupToCloud = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!backupId || !provider || !config) throw Errors.validation("Missing required fields: backupId, provider, config")
    const result = await service.uploadBackupToCloud(backupId, provider, config)
  ...
  triggerOCR = asyncHandler(async (req: Request, res: Response) => {
    ...
    res.status(202).json({ success: true, message: "OCR extraction started", data: ocrRecord })
  })
  ...
  getOCRResult = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!ocrRecord) {
      res.status(200).json({ success: true, data: null, message: "No OCR result found for this file" })
      return
    }
  ...
  searchFilesByText = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!query || typeof query !== "string") throw Errors.validation("Search query is required")
  ...
  createVersion = asyncHandler(async (req: Request, res: Response) => {
    ...
    const userId = req.user?.id as string
    if (!clinicId || !userId) throw Errors.unauthorized("Authentication required")
    const version = await service.createVersion(req.params.id, clinicId, userId, req.file)
  ...
  restoreVersion = asyncHandler(async (req: Request, res: Response) => {
    const clinicId = req.user?.clinicId as string
    if (!clinicId) throw Errors.unauthorized("Authentication required")
    const file = await service.restoreVersion(req.params.id, req.params.versionId, clinicId)
    res.status(200).json({ success: true, message: "Version restored successfully", data: file })
  })
}
