...
export class AgendaService {
  ...
  async listAppointments(clinicId: string, query: {
    dentistId?: string
    patientId?: string
    status?: any
    startTime?: any
  ...
  async createAppointment(clinicId: string, body: unknown) {
    ...
    MetricsEmitter.incrementCounter("agenda_appointment_created", "Appointments created", { clinicId, type: parsed.data.title })
    try {
  ...
  async checkConflict(clinicId: string, query: {
    dentistId: string
    startTime: string
    endTime: string
    excludeId?: string
  ...
  async listConfirmations(clinicId: string, query: { appointmentId?: string; status?: string }) {
    ...
    return this.repo.findConfirmationsByAppointmentIds(appointmentIds, query.status)
  }
  ...
  async getConfirmation(id: string, clinicId: string) {
    ...
    const appointment = await this.repo.findAppointmentById(confirmation.appointment_id, clinicId)
    if (!appointment) {
      const err = new Error("Confirmation not found") as any
      err.statusCode = 404
      throw err
  ...
  async createConfirmation(clinicId: string, body: unknown) {
    ...
    const appointment = await this.repo.findAppointmentById(parsed.data.appointment_id, clinicId)
    if (!appointment) {
      const err = new Error("Appointment not found") as any
      err.statusCode = 404
      throw err
  ...
  async updateConfirmation(id: string, clinicId: string, body: unknown) {
    ...
    const appointment = await this.repo.findAppointmentById(existing.appointment_id, clinicId)
    if (!appointment) {
      const err = new Error("Confirmation not found") as any
      err.statusCode = 404
      throw err
  ...
  async listBlockedTimes(clinicId: string, query: {
    dentistId?: string
    endDatetime?: any
    startDatetime?: any
  }) {
  ...
  async listDentistSchedules(clinicId: string, query: {
    dentistId?: string
    dayOfWeek?: number
    isActive?: boolean
  }) {
  ...
  async createDentistSchedule(clinicId: string, body: unknown) {
    ...
    return this.repo.createDentistSchedule({ ...parsed.data, clinic_id: clinicId })
  }
