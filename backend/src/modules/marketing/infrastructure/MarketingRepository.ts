...
export class MarketingRepository implements IMarketingRepository {
  ...
  async updateCampaign(id: string, data: Prisma.marketing_campaignsUpdateInput) {
  ...
  // appointments
  async findAppointmentsByDateRange(clinicId: string, start: Date, end: Date): Promise<any[]> {
  ...
  async findRecentAppointmentPatientIds(clinicId: string, since: Date): Promise<any[]> {
    return prisma.appointments.findMany({
      where: {
        clinic_id: clinicId,
  ...
  async findPatientsByBirthday(clinicId: string, month: number, day: number) {
    return prisma.$queryRaw<Array<{ patient_id: string; patient_name: string; email: string | null }>>`
