...
export class DocumentRepository implements IDocumentRepository {
  ...
  upsert(
    ...
    const upsertTransaction = this.db.transaction(() => {
      const existingRow = this.db
        .prepare(
          "SELECT * FROM documents WHERE clinic_id = ? AND source_path = ?",
        )
      ...
      if (existingRow) {
        ...
        if (existing.contentHash === contentHash) {
          ...
          this.db
            .prepare("UPDATE documents SET last_indexed = ? WHERE id = ?")
            .run(now, existing.id);
  ...
  archive(sourcePath: string): void {
    this.db
      .prepare("UPDATE documents SET is_archived = 1 WHERE source_path = ?")
      .run(sourcePath);
  }
  ...
  listAll(clinicId = "default"): MemoryDocument[] {
    const rows = this.db
      .prepare(
        "SELECT * FROM documents WHERE clinic_id = ? ORDER BY last_indexed DESC",
      )
  ...
  count(clinicId = "default"): number {
    const row = this.db
      .prepare("SELECT COUNT(*) as c FROM documents WHERE clinic_id = ?")
