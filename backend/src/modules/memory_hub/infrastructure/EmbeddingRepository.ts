...
export class EmbeddingRepository implements IEmbeddingRepository {
  ...
  searchSimilar(
    ...
    const queryVec = new Float32Array(
      queryBuffer.buffer,
      queryBuffer.byteOffset,
      queryBuffer.length / 4,
    );
  ...
  getCompressionStats(): CompressionStats {
    const totalRow = this.db
      ...
      .get() as { c: number };
    const compressedRow = this.db
      .prepare("SELECT COUNT(*) as c FROM embeddings WHERE is_compressed = 1")
      .get() as { c: number };

    const total = totalRow.c;
    const compressed = compressedRow.c;

    const spaceSaved = compressed > 0 ? compressed * 4 * 768 * 4 : 0;

    return {
      totalEmbeddings: total,
      compressedEmbeddings: compressed,
      compressionRatio:
        total > 0 ? (compressed * 4 + (total - compressed)) / total : 1,
