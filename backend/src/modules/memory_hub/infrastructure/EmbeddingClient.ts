...
export abstract class EmbeddingClient {
  ...
  protected getCached(texts: string[]): { results: (EmbeddingResult | undefined)[], uncached: { text: string; index: number; hash: string }[] } {
  ...
  protected storeInCache(uncached: { text: string; index: number; hash: string }[], embeddings: number[][], results: (EmbeddingResult | undefined)[]): EmbeddingResult[] {
...
export function validateApiKey(provider: string, apiKey?: string): void {
  ...
  if (apiKey && apiKey.length < 16) {
    throw new Error(`[SECURITY] API key appears invalid (too short) for provider: ${provider}`)
  }
}
