...
export class ApiKeyValidator {
  async validate(
    ...
    try {
      ...
      logger.error("[ApiKeyValidator] Validation failed", { provider: normalizedProvider, error: message })
      return {
        valid: false,
        provider: normalizedProvider,
        error: message,
  ...
  private async validateOllama(baseUrl?: string): Promise<ApiKeyValidationResult> {
    const endpoint = (baseUrl || process.env.AI_LOCAL_ENDPOINT || "http://localhost:11434").replace(/\/$/, "")
    ...
    if (!response.ok) {
      throw new Error(`Ollama unreachable: ${response.status} ${response.statusText}`)
    }

    logger.info("[ApiKeyValidator] Ollama validated successfully", { endpoint })
    return { valid: true, provider: "ollama" }
  }

  private async validateOpenAI(apiKey: string, baseUrl?: string): Promise<ApiKeyValidationResult> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`OpenAI validation failed: ${response.status} ${response.statusText} — ${body}`)
    }
  ...
  private async validateAnthropic(apiKey: string, baseUrl?: string): Promise<ApiKeyValidationResult> {
    ...
    if (!response.ok) {
      const body = await response.text().catch(() => "")
      throw new Error(`Anthropic validation failed: ${response.status} ${response.statusText} — ${body}`)
    }
  ...
  private async validateGoogle(apiKey: string, baseUrl?: string): Promise<ApiKeyValidationResult> {
    ...
    if (!response.ok) {
      const body = await response.text().catch(() => "")
      throw new Error(`Google validation failed: ${response.status} ${response.statusText} — ${body}`)
    }
