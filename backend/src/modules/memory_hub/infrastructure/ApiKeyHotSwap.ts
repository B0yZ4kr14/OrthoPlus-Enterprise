...
try {
  ...
  logger.warn(
    "[ApiKeyHotSwap] chokidar not available — SIGHUP will be the only hot-swap mechanism",
  );
}
...
export class ApiKeyHotSwap {
  ...
  private registerSignalHandler(): void {
    process.on("SIGHUP", () => {
      logger.info(
        "[ApiKeyHotSwap] SIGHUP received — reloading API configuration",
      );
      this.reload();
    });
  ...
  private startFileWatcher(): void {
    ...
    if (!fs.existsSync(this.envPath)) {
      logger.warn(
        `[ApiKeyHotSwap] .env file not found at ${this.envPath} — file watcher disabled`,
      );
      return;
    }
  ...
  private reload(): void {
    try {
      ...
      if (provider)
        updates.provider = provider as EmbeddingProviderConfig["provider"];
      ...
      const message = error instanceof Error ? error.message : String(error);
      logger.error(
        "[ApiKeyHotSwap] Reload failed — keeping previous configuration",
        { error: message },
      );
    }
  }
  ...
  private loadEnvFile(): void {
    try {
      if (fs.existsSync(this.envPath)) {
        ...
        if (result.error) {
          logger.warn("[ApiKeyHotSwap] dotenv reload failed", {
      ...
      logger.debug(
        "[ApiKeyHotSwap] dotenv not available — relying on external env update",
      );
    }
  }
}
