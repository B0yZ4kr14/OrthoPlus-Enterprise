...
export class SecureConfigStore {
  ...
  set(key: string, value: string): void {
    if (!this.masterKey) {
      logger.warn(`[SecureConfigStore] Storing plaintext for ${key} — MEMORY_HUB_MASTER_KEY not set (development only)`)
  ...
  get(key: string): string | null {
    const row = this.db.prepare("SELECT encrypted_value, iv, auth_tag FROM config WHERE key = ?").get(key) as
    ...
    if (!this.masterKey) {
      logger.warn(`[SecureConfigStore] Cannot decrypt ${key} — MEMORY_HUB_MASTER_KEY not set`)
      return null
    }

    try {
      const decipher = crypto.createDecipheriv(this.algorithm, this.masterKey, row.iv) as crypto.DecipherGCM
      decipher.setAuthTag(row.auth_tag)

      let decrypted = decipher.update(row.encrypted_value.toString("hex"), "hex", "utf8")
      decrypted += decipher.final("utf8")

      return decrypted
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)
      logger.error(`[SecureConfigStore] Decryption failed for ${key} — data may be corrupted or master key changed`, { error: message })
      return null
    }
  }
  ...
  private deriveMasterKey(): Buffer | null {
    ...
    if (!hexKey) {
      logger.warn("[SecureConfigStore] MEMORY_HUB_MASTER_KEY not set — encryption disabled (development only)")
      return null
    }

    if (hexKey.length !== 64) {
      logger.error("[SecureConfigStore] MEMORY_HUB_MASTER_KEY must be exactly 64 hex characters (32 bytes)")
      return null
    }
    ...
    try {
      ...
      logger.error("[SecureConfigStore] MEMORY_HUB_MASTER_KEY is not valid hex")
      return null
    }
  }
  ...
  private storePlaintext(key: string, value: string): void {
    const stmt = this.db.prepare(`
      ...
      ON CONFLICT(key) DO UPDATE SET
        ...
        updated_at = excluded.updated_at
    `)
    stmt.run(key, Buffer.from(value, "utf8"), Buffer.alloc(0), Buffer.alloc(0), Date.now())
  }
}
