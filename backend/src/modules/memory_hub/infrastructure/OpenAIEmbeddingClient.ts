  ...
  async embed(texts: string[]): Promise<EmbeddingResult[]> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`OpenAI embed failed: ${response.status} ${response.statusText} — ${errorText}`)
      }
      ...
      logger.error("[OpenAIEmbeddingClient] Embed error", { error: (error as Error).message, model: this.model, requestId: this.requestId })
      throw error instanceof Error ? error : new Error("OpenAI embedding failed")
    }
  }
}
