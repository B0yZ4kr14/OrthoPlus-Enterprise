...
export class MemoryHubController {
  ...
  search = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (this.costTrackingService) {
      ...
      const model =
        process.env.MEMORY_HUB_EMBEDDING_MODEL ||
        (provider === "ollama" ? "nomic-embed-text" : "text-embedding-3-small");
      const cost = this.costTrackingService.logCost(
        clinicId,
        query,
        provider,
        model,
      ...
      if (cost.costUsd > 0) {
        logger.info("[MemoryHubController] Search cost tracked", {
          clinicId,
  ...
  contextBrief = asyncHandler(async (req: Request, res: Response) => {
    ...
    const safeMaxTokens =
      Number.isFinite(numMaxTokens) && numMaxTokens > 0
  ...
  versions = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!sourcePath || typeof sourcePath !== "string") {
      throw Errors.validation("sourcePath is required", [
        {
          field: "sourcePath",
          message: "sourcePath query parameter is required",
          code: "required",
        },
      ]);
    }
    ...
    if (!isAllowed) {
      throw Errors.validation("Invalid sourcePath", [
        {
          field: "sourcePath",
          message: "Path must start with an allowed prefix",
          code: "invalid",
        },
      ]);
    }
  ...
  health = asyncHandler(async (req: Request, res: Response) => {
    ...
    this.metrics.memoryHub.coveragePercent.set(
      { category: "memory_hub" },
  ...
  drift = asyncHandler(async (req: Request, res: Response) => {
    ...
    const total = (severity ? totalStmt.get(severity) : totalStmt.get()) as {
      c: number;
    };
