...
const timeoutMs = parseInt(process.env.MEMORY_HUB_DRIFT_TIMEOUT || "300000", 10) // default 5min (NFR-005)
...
const timeoutId = setTimeout(() => {
  logger.error("[DriftScanWorker] Timeout exceeded", { timeoutMs, projectRoot })
  db.close()
  process.exit(1)
}, timeoutMs)
