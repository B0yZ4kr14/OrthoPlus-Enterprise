...
export class DriftDetectionService {
  ...
  private detectBrokenApiRefs(): DriftIssue[] {
    ...
    try {
      ...
      for (const route of backendRoutes) {
        const callsToCheck = route.startsWith("/api/") ? uniqueFrontendCalls : uniqueRawFrontendCalls
        const hasFrontendRef = callsToCheck.some((call) => call.startsWith(route))
      ...
      logger.warn("[DriftDetectionService] detectBrokenApiRefs failed", { error: err })
    }
  ...
  private extractBackendRoutes(): string[] {
    ...
    const routeRegex = new RegExp('app\\.use\\s*\\(\\s*["\'`](/[^"\'`]+)["\'`]', "g")
  ...
  private extractFrontendApiCalls(): string[] {
    ...
    const callRegex = new RegExp('apiClient\\.(?:get|post|put|delete|patch)(?:<[^>]+>)?\\s*\\(\\s*["\'`]([^"\'`$]+)', "g")
  ...
  private extractSpecApiRefs(): string[] {
    ...
    for (const doc of allDocs) {
      if (!doc.sourcePath.includes("specs/") && !doc.sourcePath.includes("docs/")) continue
