      ...
      try {
        ...
        while ((match = mdLinkRegex.exec(content)) !== null) {
          ...
          const targetDoc = docs.find(
            (d) =>
              d.sourcePath.endsWith(targetPath) ||
              d.sourcePath.includes(targetPath),
          );
        ...
        if (Array.isArray(related)) {
          for (const rel of related) {
            ...
            if (relPath) {
              const targetDoc = docs.find(
                (d) =>
                  d.sourcePath.endsWith(relPath) ||
                  d.sourcePath.includes(relPath),
              );
              if (targetDoc && targetDoc.id !== doc.id) {
                edges.push({
                  source: doc.id,
        ...
        logger.warn(
          `[GraphService] Failed to parse links for ${doc.sourcePath}`,
          { error: err },
        );
      }
    }
