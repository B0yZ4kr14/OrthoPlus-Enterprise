...
export class FinanceiroService {
  ...
  async listTransactions(clinicId: string, query: any) {
    const { type, status, category, payment_method, start_date, end_date } = query
  ...
  async createCashRegister(clinicId: string, body: unknown) {
    ...
    return this.repo.createCashRegister({ ...data, clinic_id: clinicId } as any)
  }
  ...
  async createContaReceber(clinicId: string, userId: string, body: unknown) {
    ...
    return this.repo.createContaReceber({ ...data, clinic_id: clinicId, created_by: userId } as any)
  }
  ...
  async createContaPagar(clinicId: string, userId: string, body: unknown) {
    ...
    return this.repo.createContaPagar({ ...data, clinic_id: clinicId, created_by: userId } as any)
  }
  ...
  async createNotaFiscal(clinicId: string, userId: string, body: unknown) {
    ...
    return this.repo.createNotaFiscal({ ...data, clinic_id: clinicId, created_by: userId } as any)
  }
  ...
  async updateExtrato(id: string, clinicId: string, body: unknown) {
    ...
    if (!existing) throw notFound()
    const { conciliado, transaction_id, observacoes } = this.validate(updateExtratoSchema, body)
  ...
  async sugerirSangriaIa(body: any) {
    ...
    return { success: true, sugestao: { valorSangria: 0, reservar: valorAtualCaixa, motivo: "Model rules currently mocked" } }
  }
  ...
  async enviarCobranca(body: any) {
    ...
    if (!contaReceberId || !method) throw badRequest("contaReceberId and method are required")
  ...
  async processarPagamento(body: any) {
    ...
    if (!contaReceberId || !amount || !paymentMethod) throw badRequest("Required fields missing")
    const result = await this.processarPagamentoUseCase.execute({ contaReceberId, amount, paymentMethod })
    return { success: true, transaction_id: result.transactionId, status: result.status }
  }
  ...
  async processarSplitPagamento(body: any) {
    ...
    if (!transactionId || !splits || !splits.length) throw badRequest("transactionId and splits mapping are required")
