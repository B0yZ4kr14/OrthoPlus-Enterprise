...
import {
  PacienteSearchService,
  SearchPacientesFilters,
} from "./services/PacienteSearchService";
...
export class PacientesControllerService {
  ...
  async patientAuth(
    ...
    if (action === "login") {
      ...
      const account =
        await this.patientRepository.findPatientAccountByEmail(email);
    ...
    if (action === "logout") {
      if (sessionIdFromCookie) {
        await this.patientRepository.deletePatientSessionsBySessionId(
          sessionIdFromCookie,
        );
      }
