...
export class InadimplenciaController {
  ...
  async updateInadimplente(req: Request, res: Response) {
    ...
    const { id } = req.params;
    const existing = await prisma.inadimplentes.findFirst({ where: { id, clinic_id: clinicId } });
  ...
  async createCampanhaCobranca(req: Request, res: Response) {
    ...
    const data = await prisma.campanhas_inadimplencia.create({
      data: { ...parsed.data, clinic_id: clinicId, status: parsed.data.status || "ATIVA", tipo_campanha: parsed.data.tipo_campanha || "manual" },
    });
  ...
  async updateCampanhaCobranca(req: Request, res: Response) {
    ...
    const { id } = req.params;
    const existing = await prisma.campanhas_inadimplencia.findFirst({ where: { id, clinic_id: clinicId } });
