export interface IFidelidadeRepository {
  ...
  upsertPacienteFidelidade(clinicId: string, patientId: string, pontos: number): Promise<any>
  findBadgesByClinic(clinicId: string): Promise<any[]>
  findPacienteFidelidade(clinicId: string, patientId: string): Promise<any | null>
  addPointsTransaction(clinicId: string, patientId: string, pontos: number, pontoData: any): Promise<[any, any, any[]]>
