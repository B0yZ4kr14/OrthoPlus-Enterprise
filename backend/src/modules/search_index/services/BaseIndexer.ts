...
export abstract class BaseIndexer<TEntity> {
  ...
  protected abstract queryBatch(cursor?: string, since?: Date): Promise<TEntity[]>
  protected abstract extractData(entity: TEntity): Promise<TEntity> | TEntity
  protected abstract buildIndexEntry(entity: TEntity): Promise<SearchIndexEntry> | SearchIndexEntry
  ...
  protected async indexBatch(items: TEntity[]): Promise<void> {
    ...
    const entries = await Promise.all(enriched.map((e) => this.buildIndexEntry(e)))
