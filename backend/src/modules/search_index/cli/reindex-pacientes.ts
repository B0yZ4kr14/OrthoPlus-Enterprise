...
const indexer = new PacienteIndexer(prisma)
...
async function main() {
  ...
  try {
    if (force) {
      console.log("[CLI] Iniciando reindexacao completa de pacientes...")
      const result = await indexer.fullReindex(true)
      console.log(`[CLI] Reindexacao completa finalizada.`)
      console.log(`[CLI] Indexados: ${result.indexed} | Duracao: ${result.durationMs}ms`)
    } else if (since) {
      console.log(`[CLI] Iniciando reindexacao incremental desde ${since.toISOString()}...`)
      ...
      console.error("[CLI] Uso: tsx reindex-pacientes.ts --force")
      console.error("[CLI] Uso: tsx reindex-pacientes.ts --since=2024-01-01T00:00:00Z")
      process.exit(1)
    }
  } catch (err) {
