import {
  IVendaRepository,
  FindAllOptions,
} from "../../domain/repositories/IVendaRepository";
...
export class VendaRepositoryPostgres implements IVendaRepository {
  async findById(id: string): Promise<Venda | null> {
    const result = await db.query("SELECT * FROM pdv.vendas WHERE id = $1", [
      id,
    ]);
