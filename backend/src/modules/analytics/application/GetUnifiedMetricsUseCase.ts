...
export class GetUnifiedMetricsUseCase {
  ...
  async execute(clinicId: string): Promise<UnifiedMetricsResult> {
    ...
    // Executive Metrics
    const [receita_total, receita_mes_anterior, despesas_total] = await Promise.all([
      this.repo.aggregateRevenue(clinicId, "RECEITA", startOfMonth),
      this.repo.aggregateRevenue(clinicId, "RECEITA", startOfLastMonth, startOfMonth),
    ...
    // Clinical Metrics
    const appointments = await this.repo.getAppointmentsForPeriod(clinicId, startOfMonth)
    const total_appointments = appointments.length
    const completed_appointments = appointments.filter((a) => a.status === "CONCLUIDA").length
    const taxa_ocupacao =
    ...
    // Financial Metrics
    const unique_patients = await this.repo.getUniquePayingPatients(clinicId, startOfMonth)
    ...
    const conversao_leads =
      total_leads && converted_leads ? (converted_leads / total_leads) * 100 : 0

    const custo_marketing = await this.repo.getMarketingExpenses(clinicId, startOfMonth)
    const custo_aquisicao = converted_leads ? custo_marketing / converted_leads : 0

    const lifetime_value = ticket_medio * 12
    const roi_marketing =
      custo_marketing > 0
        ? ((lifetime_value * converted_leads - custo_marketing) / custo_marketing) * 100
        : 0
