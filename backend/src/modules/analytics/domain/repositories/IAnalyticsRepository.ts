...
export interface IAnalyticsRepository {
  ...
  // Unified Metrics
  aggregateRevenue(clinicId: string, type: string, startDate: Date, endDate?: Date): Promise<number>
  ...
  getPendingReceivables(clinicId: string): Promise<Array<{ data_vencimento: string }>>
  ...
  // Loyalty
  findLoyaltyByPatient(clinicId: string, patientId: string): Promise<unknown | null>
  ...
  // Gamification
  findActiveGamificationGoals(clinicId: string, userId: string): Promise<unknown[]>
  countAppointmentsByDentist(dentistId: string, startMonth: Date): Promise<number>
  ...
  // Sidebar Badges
  countAppointmentsToday(clinicId: string, todayStr: string, tomorrowStr: string): Promise<number>
  countOverdueContasReceber(clinicId: string, todayStr: string): Promise<number>
  countRecallsToday(clinicId: string, todayStr: string, tomorrowStr: string): Promise<number>
}
