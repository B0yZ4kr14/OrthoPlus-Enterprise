 ...
 */
async function anonymizePatient(patientId: string, dryRun: boolean = false): Promise<void> {
  logger.info(`LGPD PURGE: Starting anonymization for patient ${patientId}`, { dryRun });
  ...
  try {
    ...
    logger.error(`LGPD PURGE: Error anonymizing patient ${patientId}`, { error });
    throw error;
  }
}
...
async function main(): Promise<void> {
  ...
  try {
    if (options.patientId) {
      ...
      console.error('Usage: npx ts-node src/scripts/lgpd-purge.ts --patientId=<id> [--dry-run]');
      process.exit(1);
    }
