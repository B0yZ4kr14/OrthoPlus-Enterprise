...
export function registerEventHandlers(): void {
  ...
  eventBus.register(
    "PDV.VendaRegistrada",
    new AtualizarEstoqueHandler(produtoRepo),
  );
  eventBus.register(
    "PDV.VendaRegistrada",
    new GerarFinanceiroHandler(transactionRepo),
  );
  eventBus.register(
    "Faturamento.NFeAutorizada",
    new NotificarClienteNFeHandler(),
  );
  eventBus.register(
    "Inventario.EstoqueAlterado",
    new VerificarEstoqueMinimoHandler(),
  );
  ...
  const searchIndexAgendaHandler = new SearchIndexAgendaHandler(
    new AgendaIndexer(prisma),
  );
  ...
  const searchIndexPepHandler = new SearchIndexPepHandler(
    new PepIndexer(prisma),
  );
