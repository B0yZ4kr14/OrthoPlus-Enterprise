...
export class SearchIndexAgendaHandler implements EventHandler<DomainEvent> {
  ...
  async handle(event: DomainEvent): Promise<void> {
    try {
      if (event instanceof AppointmentCreatedEvent || event instanceof AppointmentUpdatedEvent) {
        await this.agendaIndexer.reindexById(event.appointmentId);
        logger.debug('Agenda reindexada em tempo real', { appointmentId: event.appointmentId });
      } else if (event instanceof AppointmentDeletedEvent) {
        await this.agendaIndexer.deleteByEntityId(event.appointmentId);
        logger.debug('Entrada de agenda removida do search_index', { appointmentId: event.appointmentId });
      }
    } catch (error) {
      logger.error('Erro ao reindexar agenda em tempo real', { error, eventType: event.eventType });
