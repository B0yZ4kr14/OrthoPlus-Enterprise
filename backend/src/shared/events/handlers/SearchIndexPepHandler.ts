...
export class SearchIndexPepHandler implements EventHandler<DomainEvent> {
  ...
  async handle(event: DomainEvent): Promise<void> {
    try {
      if (event instanceof ProntuarioCreatedEvent || event instanceof ProntuarioUpdatedEvent) {
        await this.pepIndexer.reindexById(event.prontuarioId);
        logger.debug('PEP reindexado em tempo real', { prontuarioId: event.prontuarioId });
      } else if (event instanceof ProntuarioDeletedEvent) {
        await this.pepIndexer.deleteByEntityId(event.prontuarioId);
        logger.debug('Entrada de prontuario removida do search_index', { prontuarioId: event.prontuarioId });
      } else if (
      ...
      logger.error('Erro ao reindexar PEP em tempo real', { error, eventType: event.eventType });
