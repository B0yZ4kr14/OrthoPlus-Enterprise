...
export class JWTAuthService implements IAuthService {
  ...
  async login(
    ...
    // Buscar usuário
    const result = await this.db.query<any>( // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  async signup(data: SignupData): Promise<{ user: User; tokens: AuthTokens }> {
    ...
    try {
      ...
      const userResult = await tx.query<any>( // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  async verifyToken(accessToken: string): Promise<User> {
    try {
      ...
      return {
        ...
        role: payload.role as any, // eslint-disable-line @typescript-eslint/no-explicit-any
      };
    } catch (error) {
  ...
  private generateTokens(payload: JWTPayload): AuthTokens {
    const accessToken = jwt.sign(payload, this.jwtSecret, {
      expiresIn: this.jwtExpiresIn as any, // eslint-disable-line @typescript-eslint/no-explicit-any
    });

    const refreshToken = jwt.sign(payload, this.jwtSecret, {
      expiresIn: this.refreshExpiresIn as any, // eslint-disable-line @typescript-eslint/no-explicit-any
    });
