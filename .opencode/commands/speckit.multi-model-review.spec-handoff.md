---
description: Export a spec-authoring handoff prompt for the configured development
  spec model, model-specific options, and optional subagent task routing.
argument-hint: '[slug|feature brief] [--spec-model <model[:axis]@axis>] [--heavy]
  [--plan] [--subagents auto|off] [--model <id>] [--model-option <key=value>] [--dev-model
  <model[:axis]@axis>] [--implementation-model <id>] [--implementation-option <key=value>]'
allowed-tools:
- Read
- Write
- Glob
...
6. Render `.specify/templates/spec-authoring-prompt.md`.
   - Substitute:
     - `{{SPEC_AUTHOR_MODEL}}`
     - `{{SPEC_AUTHOR_OPTIONS}}`
     - `{{SPEC_AUTHOR_PROFILE}}`
