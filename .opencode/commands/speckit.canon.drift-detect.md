---
description: Detect spec-level drift — scan `tasks.drift.md` and reverse-engineer
  spec discrepancies against `spec.md`, writing only drifted items to `spec.drift.md`.
handoffs:
- label: Resolve Drift
...
**Before doing anything else**, run `bash .specify/extensions/canon/scripts/bash/check-prerequisites.sh --json --require-tasks-drift` from repo root and parse JSON for `REPO_ROOT`, `BRANCH`, `FEATURE_DIR`, `FEATURE_SPEC`, `TASKS`, `TASKS_DRIFT`, and `SPEC_DRIFT`. All paths must be absolute.
