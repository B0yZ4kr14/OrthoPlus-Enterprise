---
agent: speckit.schedule.portfolio
---
...
description: Interactively create or edit the encapsulated schedule-config.yml — the
  agent portfolio, skill inference rules, and solver parameters consumed by /speckit.schedule.run.
  Auto-scaffolds from the project's tech stack AND the AI assistant's on-disk fleet
  via solver.autodetect.
compatibility: Requires spec-kit project structure with .specify/ directory
metadata:
  author: github-spec-kit
  source: schedule:commands/portfolio.md
---
...
3. **Per-AI starter slots** — appended for any role coverage gaps.
   ...
   |-----------------|---------------------------------|-------------------------------------|
   | `claude`        | `.specify/templates/portfolio-claude.yml`  | `opus` / `sonnet` / `haiku`         |
   | `copilot`       | `.specify/templates/portfolio-copilot.yml` | `gpt4o` / `gpt4o-mini` / `o3-mini`  |
   | `cursor-agent`  | `.specify/templates/portfolio-cursor.yml`  | 5-agent multi-provider mix          |
   | `gemini`        | `.specify/templates/portfolio-gemini.yml`  | `pro` / `flash` / `flash-lite`      |
   | (other / none)  | `.specify/templates/base-portfolio.yml`    | `frontier` / `mid` / `small` (REPLACE_ME) |
...
- Auto-generated from: .kimi/skills/speckit-schedule-portfolio/SKILL.md
- Synced: 2026-05-24T21:37:05-03:00
