...
description: before_implement hook (mandatory — THE GATE). Verifies assessment + plan
  + [MIG-*] preconditions; refuses to proceed with speckit.implement otherwise. Then
  executes each unchecked [MIG-*] task in order with per-task user review (approve
  | skip | abort | autoApprove-rest), validating that every dispatch target matches
  ^speckit\.fx-to-dotnet\. Build failures always pause even under autoApprove-rest.
  Silent-exit on non-Framework solutions.
compatibility: Requires spec-kit project structure with .specify/ directory
metadata:
  author: github-spec-kit
  source: fx-to-dotnet:commands/hooks/implement-hook.md
---
...
- Auto-generated from: .kimi/skills/speckit-fx-to-dotnet-implement-hook/SKILL.md
