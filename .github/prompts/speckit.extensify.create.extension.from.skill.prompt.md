---
agent: speckit.extensify.create.extension.from.skill
---
...
description: Create a Spec Kit extension from a skill (SKILL.md and its enclosing
  directory).
...
4. Inventory every other file and subdirectory in the skill's enclosing directory (e.g. `.specify/scripts/`, `references/`, `assets/`, templates, etc.).
...
└── resources/                    # only if the skill has bundled files
    ├── .specify/scripts/                  # copied from skill
...
| `.specify/scripts/`, `references/`, `assets/`, other files | `resources/` subtree (preserving directory structure) |
...
- Rewrite relative file references (e.g. `[script](.specify/scripts/test.js)`) to point into the `resources/` directory (e.g. `[script](../resources/scripts/test.js)`).
...
- Auto-generated from: .kimi/skills/speckit-extensify-create-extension-from-skill/SKILL.md
