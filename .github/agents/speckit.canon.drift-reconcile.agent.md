---
description: Infer canon gaps — compare the resolved `spec.drift.md` against canon
  and write a canon plan to `canon.drift.md`.
handoffs:
...
**Before doing anything else**, run `bash .specify/extensions/canon/scripts/bash/check-prerequisites.sh --json --require-spec-drift --canon` from repo root and parse JSON for `REPO_ROOT`, `BRANCH`, `FEATURE_DIR`, `FEATURE_SPEC`, `TASKS_ALIGNMENT`, `SPEC_DRIFT`, `CANON_DRIFT`, `CANON_ROOT`, and `CANON_TOC`. All paths must be absolute.
