# Implementation Plan: Gestão de Arquivos e Documentos

**Branch**: `[015-files]` | **Date**: 2026-05-17 | **Spec**: [spec.md](./spec.md)

**Input**: Feature specification from `specs/015-files/spec.md`
...
This plan implements **Gestão de Arquivos e Documentos** for OrthoPlus Enterprise, a brownfield feature targeting existing module `file_management` (backend) and `filemanagement` (frontend). The approach is incremental: enhance existing Prisma models, extend Express controllers, and build/adapt React UI components with Clean Architecture where applicable.

**Primary requirement**: Armazenamento de arquivos com metadados.
...
| **Performance Goals** | API < 500ms (p99) for metadata; upload < 5s (streaming, up to 50MB); download < 1s; thumbnail/preview < 2s; OCR < 10s (background job) |
...
- [ ] Review existing `file_management` Prisma models and API
- [ ] Audit current frontend `filemanagement` components
...
- [ ] P011 [P] Create/update reusable components in `filemanagement/ui/components/`
...
#### US1: Upload de Documento (Priority: P1)
...
- [ ] US1-001 [P] UI: Upload de Documento — main view/component
...
#### US2: Visualização e Download (Priority: P1)
...
- [ ] US2-001 [P] UI: Visualização e Download — main view/component
...
#### US3: OCR e Indexação (Priority: P2)
...
- [ ] US3-001 [P] UI: OCR e Indexação — main view/component
...
- `pep` — module must be functional before this feature
- `orcamentos` — module must be functional before this feature
- `auth` — module must be functional before this feature
...
- **Brownfield**: Module `file_management` already exists — extend, don't rebuild
- **CA modules**: If `filemanagement` uses Clean Architecture, follow existing patterns (domain → application → infrastructure)
...
|-------------|-------------|----------|
| **FIL-FR-001** | Upload | ✅ Covered |
| **FIL-FR-002** | Storage | ✅ Covered |
| **FIL-FR-003** | Visualização | ✅ Covered |
| **FIL-FR-004** | Permissões | ✅ Covered |
| **FIL-FR-005** | OCR e Busca | ✅ Covered |
