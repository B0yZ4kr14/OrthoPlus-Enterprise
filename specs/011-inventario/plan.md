# Implementation Plan: Gestão de Inventário

**Branch**: `[011-inventario]` | **Date**: 2026-05-17 | **Spec**: [spec.md](./spec.md)

**Input**: Feature specification from `specs/011-inventario/spec.md`
...
This plan implements **Gestão de Inventário** for OrthoPlus Enterprise, a brownfield feature targeting existing module `inventory_management` (backend) and `inventorymanagement` (frontend). The approach is incremental: enhance existing Prisma models, extend Express controllers, and build/adapt React UI components with Clean Architecture where applicable.

**Primary requirement**: Catálogo de materiais e produtos.
...
| **Performance Goals** | API < 200ms (p99) for stock query; entry/exit < 500ms (includes stock recalculation); low-stock alerts < 1s (batch) |
...
│   │   ├── inventory_managementController.ts    # Extend existing
│   │   ├── inventory_managementService.ts       # Add business logic
│   │   ├── inventory_managementRoutes.ts        # Register endpoints
│   │   └── types/                           # DTOs, enums
│   └── prisma/schema.prisma                 # Add/extend models
├── tests/
│   └── inventory_management.test.ts             # Integration tests
...
- [ ] Review existing `inventory_management` Prisma models and API
- [ ] Audit current frontend `inventorymanagement` components
...
- [ ] P011 [P] Create/update reusable components in `inventorymanagement/ui/components/`
...
#### US1: Cadastro de Produto (Priority: P1)
...
- [ ] US1-001 [P] UI: Cadastro de Produto — main view/component
...
#### US2: Entrada e Saída (Priority: P1)
...
- [ ] US2-001 [P] UI: Entrada e Saída — main view/component
...
#### US3: Alertas de Estoque (Priority: P2)
...
- [ ] US3-001 [P] UI: Alertas de Estoque — main view/component
...
- `pdv` — module must be functional before this feature
...
- **Brownfield**: Module `inventory_management` already exists — extend, don't rebuild
- **CA modules**: If `inventorymanagement` uses Clean Architecture, follow existing patterns (domain → application → infrastructure)
...
|-------------|-------------|----------|
| **INV-FR-001** | CRUD de Produtos | ✅ Covered |
| **INV-FR-002** | Movimentações | ✅ Covered |
| **INV-FR-003** | Alertas | ✅ Covered |
| **INV-FR-004** | Relatórios | ✅ Covered |
| **INV-FR-005** | Integração com PDV | ✅ Covered |
