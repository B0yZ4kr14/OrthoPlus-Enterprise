...
| GAP-3 | ~~Backend controller doesn't use domain commands~~ | ✅ Resolved — `createAppointment` now uses `CreateAppointmentCommandHandler` + `AppointmentRepositoryPostgres` on 2026-05-23. |
...
**Source**: Post-implementation cleanup of agenda brownfield gaps
...
- [x] TD002 [P] `agendaController.ts` — respostas de erro 500 retornam `{ error: "Internal server error" }` em vez de RFC 7807 Problem Details (CQ-3). ~~Deve propagar erros para o `errorHandler` middleware~~ — Fixed on 2026-05-24. All 20 `res.status(500).json(...)` replaced with `next(error)`. Controllers now delegate to RFC 7807 `errorHandler` middleware.
...
- [x] T080 [P2] Add Prometheus histogram `appointment_create_duration_ms` in `CreateAppointmentCommandHandler`
- [x] T081 [P2] Add Prometheus histogram `calendar_load_duration_ms` in `getAppointments` controller
- [x] T082 [P2] Add `appointments_total` counter with labels for `status` and `clinic_id`
- [x] T083 [P3] Create Grafana dashboard panel for agenda latency metrics
...
**Generated**: 2026-05-24
**Source**: Post-implementation cleanup of agenda instrumentation (T080–T082)
**Priority**: Address before next feature iteration
