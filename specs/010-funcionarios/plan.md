# Implementation Plan: Gestão de Funcionários

**Branch**: `[010-funcionarios]` | **Date**: 2026-05-17 | **Spec**: [spec.md](./spec.md)

**Input**: Feature specification from `specs/010-funcionarios/spec.md`
...
This plan implements **Gestão de Funcionários** for OrthoPlus Enterprise, a brownfield feature targeting existing module `staff_management` (backend) and `staffmanagement` (frontend). The approach is incremental: enhance existing Prisma models, extend Express controllers, and build/adapt React UI components with Clean Architecture where applicable.

**Primary requirement**: Gestão completa de colaboradores.
...
| **Performance Goals** | API < 300ms (p99) for CRUD; time clock < 200ms; commission calculation < 2s (monthly batch) |
...
│   │   ├── staff_managementController.ts    # Extend existing
│   │   ├── staff_managementService.ts       # Add business logic
│   │   ├── staff_managementRoutes.ts        # Register endpoints
│   │   └── types/                           # DTOs, enums
│   └── prisma/schema.prisma                 # Add/extend models
├── tests/
│   └── staff_management.test.ts             # Integration tests
...
- [ ] Review existing `staff_management` Prisma models and API
- [ ] Audit current frontend `staffmanagement` components
...
- [ ] P011 [P] Create/update reusable components in `staffmanagement/ui/components/`
...
#### US1: Cadastro de Funcionário (Priority: P1)
...
- [ ] US1-001 [P] UI: Cadastro de Funcionário — main view/component
...
#### US2: Controle de Ponto (Priority: P1)
...
- [ ] US2-001 [P] UI: Controle de Ponto — main view/component
...
#### US3: Escalas e Folgas (Priority: P2)
...
- [ ] US3-001 [P] UI: Escalas e Folgas — main view/component
...
- `files` — module must be functional before this feature
...
- **Brownfield**: Module `staff_management` already exists — extend, don't rebuild
- **CA modules**: If `staffmanagement` uses Clean Architecture, follow existing patterns (domain → application → infrastructure)
...
|-------------|-------------|----------|
| **FUN-FR-001** | CRUD de Funcionários | ✅ Covered |
| **FUN-FR-002** | Controle de Ponto | ✅ Covered |
| **FUN-FR-003** | Escala de Trabalho | ✅ Covered |
| **FUN-FR-004** | Comissões e Metas | ✅ Covered |
| **FUN-FR-005** | Documentos | ✅ Covered |
