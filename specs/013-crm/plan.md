# Implementation Plan: CRM e Marketing

**Branch**: `[013-crm]` | **Date**: 2026-05-17 | **Spec**: [spec.md](./spec.md)

**Input**: Feature specification from `specs/013-crm/spec.md`
...
This plan implements **CRM e Marketing** for OrthoPlus Enterprise, a brownfield feature targeting existing module `crm_marketing` (backend) and `crmmarketing` (frontend). The approach is incremental: enhance existing Prisma models, extend Express controllers, and build/adapt React UI components with Clean Architecture where applicable.

**Primary requirement**: Criação e execução de campanhas de marketing.
...
| **Performance Goals** | API < 300ms (p99) for queries; recall campaign batch < 5s (background job); conversion funnel < 2s; NPS survey < 300ms |
...
│   │   ├── crm_marketingController.ts    # Extend existing
│   │   ├── crm_marketingService.ts       # Add business logic
│   │   ├── crm_marketingRoutes.ts        # Register endpoints
│   │   └── types/                           # DTOs, enums
│   └── prisma/schema.prisma                 # Add/extend models
├── tests/
│   └── crm_marketing.test.ts             # Integration tests

apps/web/
├── src/
│   ├── modules/crmmarketing/
...
- [ ] Review existing `crm_marketing` Prisma models and API
- [ ] Audit current frontend `crmmarketing` components
...
- [ ] P011 [P] Create/update reusable components in `crmmarketing/ui/components/`
...
#### US1: Campanha de Recall (Priority: P1)
...
- [ ] US1-001 [P] UI: Campanha de Recall — main view/component
...
#### US2: Aniversariantes (Priority: P1)
...
- [ ] US2-001 [P] UI: Aniversariantes — main view/component
...
#### US3: Funil de Conversão (Priority: P2)
...
- [ ] US3-001 [P] UI: Funil de Conversão — main view/component
...
- **Brownfield**: Module `crm_marketing` already exists — extend, don't rebuild
- **CA modules**: If `crmmarketing` uses Clean Architecture, follow existing patterns (domain → application → infrastructure)
...
|-------------|-------------|----------|
| **CRM-FR-001** | Campanhas | ✅ Covered |
| **CRM-FR-002** | Automações | ✅ Covered |
| **CRM-FR-003** | Funil de Conversão | ✅ Covered |
| **CRM-FR-004** | Pesquisa de Satisfação (NPS) | ✅ Covered |
| **CRM-FR-005** | Indicações | ✅ Covered |
