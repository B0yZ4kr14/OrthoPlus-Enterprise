...
test.describe("Module Permissions E2E", () => {
  ...
  test("MEMBER without permission should not see restricted modules", async ({
    page,
  }) => {
  ...
  test("Direct access to route without permission should redirect to /403", async ({
    page,
  }) => {
  ...
  test("Module configuration page should be accessible only by ADMIN", async ({
    page,
  }) => {
  ...
  test("Enabling a module should make it appear in navigation", async ({
    page,
  }) => {
    // Navigate to module management
    await page.goto("/administracao/modulos");
  ...
  test("Module dependencies should be validated before disabling", async ({
    page,
  }) => {
    await page.goto("/administracao/modulos");
    await page.waitForLoadState("domcontentloaded");
...
test.describe("Multi-Clinic Access", () => {
  test("ADMIN with multiple clinics can switch between them", async ({
    page,
  }) => {
    // Auth token injected via fixtures.ts
    await page.goto("/");
