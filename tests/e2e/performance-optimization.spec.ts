...
test.describe("Performance Optimization - React.memo & useCallback", () => {
  ...
  test.describe("PatientsList - React.memo optimization", () => {
    test("should render patients list without excessive re-renders", async ({
      page,
    }) => {
    ...
    test("should handle search filter efficiently with useCallback", async ({
      page,
    }) => {
      await page.goto("/pacientes");
      await expect(page.locator("table")).toBeVisible({ timeout: 10000 });
    ...
    test("should handle status filter with memoized callback", async ({
      page,
    }) => {
      await page.goto("/pacientes");
      await expect(page.locator("table")).toBeVisible({ timeout: 10000 });
    ...
    test("should handle delete confirmation with memoized handler", async ({
      page,
    }) => {
      await page.goto("/pacientes");
      await expect(page.locator("table")).toBeVisible({ timeout: 10000 });
      ...
      const deleteButton = page
        .locator('button[aria-label*="Excluir"], button')
      ...
      if ((await deleteButton.count()) > 0) {
        ...
        // Dialog should close
        await expect(
          page.locator("text=/Confirmar exclusão/i"),
        ).not.toBeVisible();
      }
    });
  });
  ...
  test.describe("TransactionsList - React.memo optimization", () => {
    ...
    test("should format currency with memoized useCallback", async ({
      page,
    }) => {
      await page.goto("/financeiro/transacoes");
      await page.waitForLoadState("domcontentloaded");
    ...
    test("should handle view details with memoized callback", async ({
      page,
    }) => {
      await page.goto("/financeiro/transacoes");
      await page.waitForLoadState("domcontentloaded");
  ...
  test.describe("DentistasList - React.memo optimization", () => {
    test("should render dentists list without excessive re-renders", async ({
      page,
    }) => {
    ...
    test("should handle search with useCallback optimization", async ({
      page,
    }) => {
      await page.goto("/dentistas");
      await page.waitForLoadState("domcontentloaded");
  ...
  test.describe("ProcedimentosList - useMemo & useCallback optimization", () => {
    test("should render procedures with memoized filters", async ({ page }) => {
      ...
      const hasContent =
        (await page.locator("text=/Procedimento/i").count()) > 0;
  ...
  test.describe("Performance Metrics", () => {
    test("should load patients page within acceptable time", async ({
      page,
    }) => {
      const startTime = Date.now();

      await page.goto("/pacientes");
      await expect(page.locator("table, text=/Nenhum/i")).toBeVisible({
        timeout: 5000,
      });
