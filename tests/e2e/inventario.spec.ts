...
test.describe("Inventory Management", () => {
  ...
  test("should display inventory list", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /inventário de estoque/i }),
    ).toBeVisible();
  ...
  test("should create new inventory", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /novo inventário/i }),
    ).toBeVisible();
    ...
    await page
      ...
      .fill("Inventário criado por teste E2E");
  ...
  test("should open counting dialog for in-progress inventory", async ({
    page,
  }) => {
    // Search for in-progress inventory
    const rowEmAndamento = page
      .locator("tbody tr")
      .filter({ hasText: /em andamento/i })
      .first();
    ...
    if (await rowEmAndamento.isVisible()) {
      ...
      await expect(
        page.getByRole("heading", { name: /contagem de inventário/i }),
      ).toBeVisible();

      // Check that items table exists
      await expect(
        page.getByRole("columnheader", { name: /qtd\. física/i }),
      ).toBeVisible();
    }
  });
  ...
  test("should view discrepancies of completed inventory", async ({ page }) => {
    ...
    if (await rowConcluido.isVisible()) {
      ...
      const divergenciasBtn = rowConcluido.getByRole("button", {
        name: /divergências/i,
      });
      ...
      if (await divergenciasBtn.isVisible()) {
        ...
        await expect(
          page.getByRole("heading", { name: /divergências do inventário/i }),
        ).toBeVisible();
  ...
  test("should allow editing non-completed inventory", async ({ page }) => {
    ...
    const rowEditavel = page
      .locator("tbody tr")
      .filter({ hasText: /(planejado|em andamento)/i })
      .first();
    ...
    if (await rowEditavel.isVisible()) {
      ...
      await expect(
        page.getByRole("heading", { name: /editar inventário/i }),
      ).toBeVisible();
  ...
  test("should show counting progress", async ({ page }) => {
    ...
    const progressBars = page
      .locator('[role="progressbar"], .bg-primary')
      .filter({ has: page.locator(".bg-secondary") });
  ...
  test("should display alerts for critical discrepancies", async ({ page }) => {
    ...
    const alertIcons = page.locator(
      '[title*="divergências"], .text-orange-500, .text-red-500',
    );

    // Inventories with discrepancies should have visual indicators
    const divergenciasCount = await page
      .locator("tbody tr")
      .filter({ has: alertIcons })
      .count();
  ...
  test("should allow exporting discrepancy report", async ({ page }) => {
    ...
    if (await rowComDivergencias.isVisible()) {
      ...
      const divergenciasBtn = rowComDivergencias.getByRole("button", {
        name: /divergências/i,
      });
