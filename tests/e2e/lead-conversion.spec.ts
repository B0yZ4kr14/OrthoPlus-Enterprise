...
test.describe("Lead Conversion E2E", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.evaluate(() => {
      localStorage.setItem(
        "sb-yxpoqjyfgotkytwtifau-auth-token",
        JSON.stringify({
  ...
  test("should create lead and convert to patient", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="lead-status"]')).toContainText(
      "QUALIFICADO",
    );
  ...
  test("should track lead activities", async ({ page }) => {
    ...
    ).toContainText("Ligação realizada");
  });
  ...
  test("should show lead funnel statistics", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="stage-qualificado"]'),
    ).toBeVisible();
    await expect(page.locator('[data-testid="stage-proposta"]')).toBeVisible();
    await expect(
      page.locator('[data-testid="stage-convertido"]'),
    ).toBeVisible();

    // Verify conversion rate calculation
    const conversionRate = await page
      .locator('[data-testid="conversion-rate"]')
      .textContent();
    expect(conversionRate).toMatch(/\d+%/);

    // Verify average time in funnel
    const avgTime = await page
      .locator('[data-testid="avg-conversion-time"]')
      .textContent();
