...
test.describe("PEP (Electronic Health Record) Workflows", () => {
  ...
  test("should create new EHR for patient", async ({ page }) => {
    ...
    await page.fill(
      'textarea[name="historico_doenca_atual"]',
      "Paciente relata dor há 3 dias",
    );
  ...
  test("should fill anamnese form", async ({ page }) => {
    ...
    await page.fill(
      'textarea[name="medicamentos_uso_continuo"]',
      "Metformina 850mg, Losartana 50mg",
    );
  ...
  test("should add treatment plan", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Restauração em Resina Composta"),
    ).toBeVisible();
  });
  ...
  test("should record clinical evolution", async ({ page }) => {
    ...
    await page.fill(
      ...
      "Paciente retornou para avaliação. Dor reduzida. Seguir com tratamento.",
    );
    await page.fill(
      'input[name="procedimento_realizado"]',
      "Consulta de retorno",
    );
  ...
  test("should upload attachment to EHR", async ({ page }) => {
    ...
    await page.setInputFiles(
      'input[type="file"]',
      "./e2e/fixtures/test-xray.jpg",
    );

    await page.fill(
      'input[name="descricao_anexo"]',
      "Radiografia periapical dente 36",
    );
    ...
    await expect(
      page.locator("text=Radiografia periapical dente 36"),
    ).toBeVisible();
  });
