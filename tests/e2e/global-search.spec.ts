...
test.describe("Global Search", () => {
  ...
  test("should display search results for patient query", async ({ page }) => {
    ...
    await page.waitForFunction(
      () => {
        ...
        const emptyState = document.body.innerText.includes(
    ...
    if (count === 0) {
      test.skip(
        true,
        "No search data available — skipping patient result assertion",
      );
    }
  ...
  test("should filter results by module", async ({ page }) => {
    ...
    const moduleBadges = page.locator(
      "[cmdk-group-heading] >> text=/Pacientes|Agendamentos|Prontuários/i",
    );
    ...
    if (badgeCount === 0) {
      test.skip(
        true,
        "No search data available — skipping module filter assertion",
      );
    }
  ...
  test("should navigate to patient detail on result click", async ({
    page,
  }) => {
    await page.locator('button[aria-label="Abrir busca global"]').click();
    await expect(page.locator("role=dialog")).toBeVisible();
    ...
    const patientGroup = page
      .locator("[cmdk-group]")
      .filter({ hasText: /Pacientes/i });
    ...
    if (count === 0) {
      test.skip(
        true,
        "No patient search results available — skipping navigation assertion",
      );
    }
