...
test.describe("WCAG AA Accessibility Validation", () => {
  ...
  test.describe("Color Contrast WCAG AA (4.5:1)", () => {
    test("should have WCAG AA compliant colors on Dashboard", async ({
      page,
    }) => {
      await page.goto("/");
    ...
    test("should have compliant badge colors - success variant", async ({
      page,
    }) => {
    ...
    test("should have compliant badge colors - warning variant", async ({
      page,
    }) => {
      ...
      const warningElements = page
        .locator('[class*="warning"], [class*="badge"]')
    ...
    test("should have compliant badge colors - error/destructive variant", async ({
      page,
    }) => {
      ...
      const errorElements = page
        .locator('[class*="destructive"], [class*="badge"]')
        .filter({ hasText: /Cancelado/i });
      const count = await errorElements.count();

      expect(count).toBeGreaterThanOrEqual(0); // May or may not have error badges
    });

    test("should have sufficient contrast in all status badges", async ({
      page,
    }) => {
  ...
  test.describe("Keyboard Navigation", () => {
    test("should allow tab navigation through interactive elements", async ({
      page,
    }) => {
      await page.goto("/");
  ...
  test.describe("Full Page Accessibility Scans", () => {
    ...
    for (const route of routes) {
      test(`should have no WCAG AA violations on ${route}`, async ({
        page,
      }) => {
        await page.goto(route);
        await page.waitForLoadState("domcontentloaded");
