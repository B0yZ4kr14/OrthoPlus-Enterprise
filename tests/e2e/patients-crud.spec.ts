...
test.describe("Patients CRUD Flow", () => {
  ...
  test("should edit existing patient", async ({ page }) => {
    await page.click(
      '[data-testid="patient-row"]:first-child [data-testid="edit-button"]',
    );
    await page.waitForSelector('[data-testid="patient-form"]');
  ...
  test("should change patient status", async ({ page }) => {
    await page.click(
      '[data-testid="patient-row"]:first-child [data-testid="status-button"]',
    );
    ...
    await page.fill(
      ...
      "Mudança de status via E2E test",
    );
  ...
  test("should add CRM data to patient", async ({ page }) => {
    ...
    await page.fill(
      'textarea[name="observacoes_comerciais"]',
      "Lead qualificado, orçamento alto",
    );
  ...
  test("should delete patient", async ({ page }) => {
    const firstPatientName = await page
      .locator('[data-testid="patient-row"]:first-child td:nth-child(2)')
      .textContent();

    await page.click(
      '[data-testid="patient-row"]:first-child [data-testid="delete-button"]',
    );
  ...
  test("should validate required fields on create", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Nome completo é obrigatório"),
    ).toBeVisible();
