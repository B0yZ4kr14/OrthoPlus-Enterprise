...
test.describe("Patient Workflow E2E", () => {
  ...
  test("should create patient with marketing data and canonical status", async ({
    page,
  }) => {
  ...
  test("should use global search with Cmd+K", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="global-search-modal"]'),
    ).toBeVisible();
  ...
  test("should display dynamic sidebar badges", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="badge-appointments"]'),
    ).toBeVisible();
    await expect(page.locator('[data-testid="badge-overdue"]')).toBeVisible();
    await expect(
      page.locator('[data-testid="badge-defaulters"]'),
    ).toBeVisible();
    await expect(page.locator('[data-testid="badge-recalls"]')).toBeVisible();

    // Check numeric values
    const appointmentsBadge = await page.textContent(
      '[data-testid="badge-appointments"]',
    );
  ...
  test("should use quick actions", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="quick-action-new-patient"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="quick-action-schedule"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="quick-action-new-sale"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="quick-action-new-budget"]'),
    ).toBeVisible();
  ...
  test("should view marketing ROI dashboard", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="kpi-conversion-rate"]'),
    ).toBeVisible();

    // Check campaign ROI chart
    await expect(
      page.locator('[data-testid="chart-campaign-roi"]'),
    ).toBeVisible();

    // Check source performance table
    await expect(
      page.locator('[data-testid="table-source-performance"]'),
    ).toBeVisible();
  });
