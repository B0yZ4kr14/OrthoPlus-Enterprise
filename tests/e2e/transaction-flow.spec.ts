...
test.describe("Transaction Flow E2E", () => {
  ...
  test("should filter transactions by period", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="period-badge"]')).toContainText(
      "Mês Passado",
    );
  });
  ...
  test("should calculate correct cash flow", async ({ page }) => {
    ...
    const receitas = await page
      .locator('[data-testid="total-receitas"]')
      .textContent();
    const despesas = await page
      .locator('[data-testid="total-despesas"]')
      .textContent();
