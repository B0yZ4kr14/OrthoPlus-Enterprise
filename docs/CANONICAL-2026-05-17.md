...
## 15. Novas Funcionalidades Implementadas (2026-05-17)

Ver documentação detalhada em [`docs/SESSION-2026-05-17.md`](./SESSION-2026-05-17.md).

### 15.1 Pacientes — Photo Upload (FR-004)
- Upload de foto do paciente via `PatientPhotoUpload` component
- Backend: `photo_url` no Prisma schema + domain entity + repository

### 15.2 Orçamentos — Scaffolding (Spec 006)
- Backend: `OrcamentoService` com CRUD + workflow (enviar/aprovar/rejeitar)
- Frontend: `OrcamentoFormPage` com PatientSelector, rotas `/orcamentos/novo` e `/orcamentos/editar/:id`

### 15.3 Database — Categoria Master + Federation Hub
- `MasterDatabaseManager` agregando 6 categorias de BD
- Endpoints: `/categories`, `/master/health`, `/master/stats`, `/master/cross-query`
- Frontend: `DatabaseManagementPage` com dropdown de categoria + visão MASTER

### 15.4 Database — Circuit Breaker + Backup Scheduler
- `CategoryCircuitBreaker` com estados CLOSED/OPEN/HALF_OPEN
- `BackupSchedulerService` unificado para todas as categorias
- Endpoints: `/circuit/metrics`, `/master/backups`, `/master/backup/:category`

### 15.5 Observability — Prometheus + Grafana + Alertmanager
- 12 métricas customizadas expostas em `/metrics`
- Grafana Dashboard: 9 panels (`orthoplus-database.json`)
- 8 alertas configurados (`prometheus-alerts.yml`)
