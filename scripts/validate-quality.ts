...
function validateSecurity() {
  ...
  try {
    migrationFiles = readdirSync(migrationsDir).filter((f) =>
      f.endsWith(".sql"),
    );
  } catch {
  ...
  migrationFiles.forEach((file) => {
    ...
    const functions =
      content.match(/CREATE( OR REPLACE)? FUNCTION[\s\S]*?\$\$/g) || [];
  ...
  results.push({
    ...
    details:
      functionsWithoutSearchPath > 0
        ? "Adicione SET search_path = public, pg_temp"
        : undefined,
  });
  ...
  results.push({
    ...
    details:
      tablesWithoutRLS > 0
        ? "Habilite RLS em todas as tabelas sensíveis"
        : undefined,
  });
}
...
function validatePerformance() {
  ...
  const componentFiles = getFilesRecursive(
  ...
  const memoPercentage = Math.round(
    (componentsWithMemo / totalComponents) * 100,
  );
  results.push({
    ...
    details:
      memoPercentage < 30
        ? "Considere adicionar React.memo em componentes pesados"
        : undefined,
  });

  // 2.2 Verificar queries com JOIN
  const repoFiles = getFilesRecursive(
    join(process.cwd(), "src", "infrastructure", "repositories"),
    ".ts",
  );
  ...
  const joinPercentage =
    totalQueries > 0 ? Math.round((queriesWithJoin / totalQueries) * 100) : 0;
  results.push({
    category: "Performance",
    status: joinPercentage >= 40 ? "PASS" : "WARN",
    message: `Queries com JOIN: ${queriesWithJoin}/${totalQueries} (${joinPercentage}%)`,
    details:
      joinPercentage < 40 ? "Otimize queries para evitar N+1" : undefined,
  });
}
  ...
  results.push({
    ...
    details:
      missingDirs.length > 0
        ? `Faltando: ${missingDirs.join(", ")}`
        : undefined,
  });

  // 3.2 Verificar use cases
  const useCaseFiles = getFilesRecursive(
    join(process.cwd(), "src", "application", "use-cases"),
    ".ts",
  );
  results.push({
    category: "Architecture",
...
function validateDocumentation() {
  ...
  try {
    ...
    results.push({
      ...
      details:
        readme.length <= 1000
          ? "README muito curto, adicione mais informações"
          : undefined,
    });
  } catch {
    results.push({
