...
export class RadiografiAnalise {
  ...
  get tipoRadiografia() {
    return this.props.tipoRadiografia;
  }
  get statusAnalise() {
    return this.props.statusAnalise;
  }
  get resultadoIa() {
    return this.props.resultadoIa;
  }
  get confidenceScore() {
    return this.props.confidenceScore;
  }
  get revisadoPorDentista() {
    return this.props.revisadoPorDentista;
  }
  ...
  concluirAnalise(
    ...
    const temProblemasCriticos = resultado.problemas.some(
      (p) => p.severidade === "critica",
    );
  ...
  calcularAcuracia(): number {
    ...
    if (
      this.props.revisadoPorDentista &&
      this.props.feedbackRating &&
      this.props.feedbackRating >= 4
    ) {
      score *= 1.1;
    }
...
export class LaudoTemplate {
  ...
  get nomeTemplate() {
    return this.props.nomeTemplate;
  }
  get templateMarkdown() {
    return this.props.templateMarkdown;
  }
  ...
  gerarLaudo(variaveis: Record<string, unknown>): string {
