...
function TestConsumer() {
  ...
  return (
    <div>
      ...
      <div data-testid="patient">{auth.isPatient ? "patient" : "not-patient"}</div>
...
describe("AuthContext", () => {
  ...
  describe("session initialization", () => {
    ...
    it("should set user but leave role null until fetchUserMetadata when /auth/me returns user without role", async () => {
      ...
      mockGet.mockImplementation((url: string) => {
        if (url === "/auth/me") {
          return Promise.resolve({ user: { id: "user-nr" }, session: "sess-nr" })
        }
  ...
  describe("signIn", () => {
    ...
    it("should show toast.error and return error object on signIn failure", async () => {
      ...
      await waitFor(() =>
        expect(screen.getByTestId("last-result").textContent).toContain("error"),
      )
  ...
  describe("signInPatient", () => {
    it("should set user, session, and profile to PATIENT on success", async () => {
      ...
      expect(toast.success).toHaveBeenCalledWith("Bem-vindo ao Portal do Paciente!")
    })
  ...
  describe("signUp", () => {
    it("should show toast.success and return error null on success", async () => {
      ...
      await waitFor(() =>
        expect(screen.getByTestId("last-result").textContent).toBe('{"error":null}'),
      )
  ...
  describe("registerStaffUser", () => {
    it("should return user and error null on success", async () => {
      mockGet.mockResolvedValueOnce({ user: null, session: null })
      mockPost.mockResolvedValueOnce({ user: { id: "staff-1", email: "s@b.com" } })
      ...
      await waitFor(() =>
        expect(screen.getByTestId("last-result").textContent).toContain("staff-1"),
      )
  ...
  describe("fetchUserMetadata", () => {
    it("should set permissions [ALL] and fetch active modules for ADMIN", async () => {
      ...
      expect(screen.getByTestId("role").textContent).toBe("ADMIN")
      expect(screen.getByTestId("modules").textContent).toBe('["agenda","financeiro"]')
    })
    ...
    it("should set permissions from API and fetch active modules for MEMBER", async () => {
      ...
      await waitFor(() =>
        expect(screen.getByTestId("permissions").textContent).toBe('["agenda","pacientes"]'),
      )
  ...
  describe("switchClinic", () => {
    it("should update selectedClinic, clinicId, fetch active modules, and toast on valid clinicId", async () => {
      function SwitchTestConsumer() {
        ...
        return (
          <div>
            <div data-testid="loading">{auth.loading ? "loading" : "ready"}</div>
            <div data-testid="clinic">{auth.clinicId || "no-clinic"}</div>
            <div data-testid="selected-clinic">{auth.selectedClinic?.name || "none"}</div>
            <div data-testid="modules">{JSON.stringify(auth.activeModules)}</div>
            <button data-testid="switch-1" onClick={() => auth.switchClinic("clinic-1")}>
              Switch 1
            </button>
          </div>
        )
      ...
      expect(screen.getByTestId("selected-clinic").textContent).toBe("Clinic One")
      expect(toast.success).toHaveBeenCalledWith("Clínica alterada para: Clinic One")
    })
    ...
    it("should return true for ADMIN_ONLY when ADMIN", async () => {
      function ModuleCheckConsumer() {
        ...
        return (
          <div>
            <div data-testid="loading">{auth.loading ? "loading" : "ready"}</div>
            <div data-testid="admin-only">{String(auth.hasModuleAccess("ADMIN_ONLY"))}</div>
          </div>
        )
      }
    ...
    it("should return true for ADMIN when activeModules is empty (fallback)", async () => {
      function ModuleCheckConsumer() {
        ...
        return (
          <div>
            <div data-testid="loading">{auth.loading ? "loading" : "ready"}</div>
            <div data-testid="agenda">{String(auth.hasModuleAccess("agenda"))}</div>
          </div>
        )
      }
    ...
    it("should return false for ADMIN when module is inactive", async () => {
      function ModuleCheckConsumer() {
        ...
        return (
          <div>
            <div data-testid="loading">{auth.loading ? "loading" : "ready"}</div>
            <div data-testid="inactive">{String(auth.hasModuleAccess("inactive"))}</div>
          </div>
        )
      }
    ...
    it("should return false for MEMBER when module is active but no permission", async () => {
      function ModuleCheckConsumer() {
        ...
        return (
          <div>
            <div data-testid="loading">{auth.loading ? "loading" : "ready"}</div>
            <div data-testid="financeiro">{String(auth.hasModuleAccess("financeiro"))}</div>
          </div>
        )
      }
    ...
    it("should use case-insensitive matching for module keys", async () => {
      function ModuleCheckConsumer() {
        ...
        return (
          <div>
            <div data-testid="loading">{auth.loading ? "loading" : "ready"}</div>
            <div data-testid="upper">{String(auth.hasModuleAccess("AGENDA"))}</div>
            <div data-testid="lower">{String(auth.hasModuleAccess("agenda"))}</div>
          </div>
        )
      }
