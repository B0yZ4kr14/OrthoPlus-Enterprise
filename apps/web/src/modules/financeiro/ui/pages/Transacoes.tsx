...
export default function Transacoes() {
  ...
  const taxaAprovacao =
    (mockTransacoes.filter((t) => t.status === "aprovado").length /
      totalTransacoes) *
    100;

  // Métodos mais usados
  const metodosCount = mockTransacoes.reduce(
    (acc, t) => {
  ...
  const metodoMaisUsado = Object.entries(metodosCount).sort(
    (a, b) => b[1] - a[1],
  )[0][0];

  // Dados para gráfico de métodos
  const metodosChartData = Object.entries(metodosCount).map(
    ([name, value]) => ({
      name,
      value,
      color:
  ...
  const volumeData = mockTransacoes.reduce(
    (acc, t) => {
  ...
  const filteredTransacoes = mockTransacoes.filter((t) => {
    const matchesSearch = t.paciente
      .toLowerCase()
                  ...
                  <TableRow key={transacao.id}>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        {format(transacao.data, "dd/MM/yyyy HH:mm", {
                          locale: ptBR,
                        })}
                      </div>
                    </TableCell>
