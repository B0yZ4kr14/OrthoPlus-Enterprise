...
describe("useCashFlow", () => {
  ...
  it("should refetch when period changes", async () => {
    const period1 = Period.custom(
      new Date("2024-01-01"),
      new Date("2024-01-31"),
    );
    const period2 = Period.custom(
      new Date("2024-02-01"),
      new Date("2024-02-29"),
    );
    ...
    await waitFor(() =>
      expect(result.current.cashFlow?.totalReceitas).toBe(5000),
    );
