...
describe("useCategories", () => {
  ...
  it("should show loading state and fetch categories on mount", async () => {
    ...
    expect(mockGet).toHaveBeenCalledWith("/financeiro/categories", expect.any(Object))
  })
      ...
      if (url === "/financeiro/categories") {
        ...
        if (params.type === "DESPESA") {
          return Promise.resolve([
            createMockCategoryRow({ id: "c2", name: "Aluguel", type: "DESPESA" }),
          ])
        }
        return Promise.resolve([
  ...
  it("should throw error when category name already exists", async () => {
    mockGet.mockResolvedValueOnce([])
    mockGet.mockResolvedValueOnce([createMockCategoryRow({ name: "Consultas", type: "RECEITA" })])
