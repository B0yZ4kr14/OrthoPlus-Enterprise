...
function makeApiOrcamento(status: string, overrides: Record<string, unknown> = {}) {
  return {
    id: "orc-1",
...
describe("useOrcamentos (presentation)", () => {
  ...
  it("should enviar an orcamento and reload list", async () => {
    ...
    mockGet.mockResolvedValueOnce([makeApiOrcamento("PENDENTE", { status: "PENDENTE" })])
    ...
    expect(mockPut).toHaveBeenCalledWith("/orcamentos/orc-1", expect.any(Object))
  ...
  it("should aprovar an orcamento and reload list", async () => {
    ...
    mockGet.mockResolvedValueOnce(makeApiOrcamento("PENDENTE"))
    mockPut.mockResolvedValueOnce({})
    mockGet.mockResolvedValueOnce([makeApiOrcamento("APROVADO", { status: "APROVADO" })])
  ...
  it("should filter orcamentos by status", async () => {
    ...
    expect(mockGet).toHaveBeenLastCalledWith("/orcamentos", {
      params: { clinic_id: "clinic-1", status: "PENDENTE", sort: "created_at.desc" },
    })
  })
