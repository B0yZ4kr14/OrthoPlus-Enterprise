...
export function useItensOrcamento(orcamentoId: string | null) {
  ...
  const itemRepository = container.resolve<IItemOrcamentoRepository>(
    SERVICE_KEYS.ITEM_ORCAMENTO_REPOSITORY,
  );
  const addItemUseCase = container.resolve<AddItemOrcamentoUseCase>(
  ...
  const addItemMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["orcamento-itens", variables.orcamentoId],
      });

      // Invalidar cache do orçamento (valores foram recalculados)
      queryClient.invalidateQueries({
        queryKey: ["orcamento", variables.orcamentoId],
      });
