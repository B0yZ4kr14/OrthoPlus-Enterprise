...
export function useOrcamentos(clinicId: string) {
  ...
  const orcamentoRepository = container.resolve<IOrcamentoRepository>(
    SERVICE_KEYS.ORCAMENTO_REPOSITORY,
  );
  const createUseCase = container.resolve<CreateOrcamentoUseCase>(
    SERVICE_KEYS.CREATE_ORCAMENTO_USE_CASE,
  );
  const updateUseCase = container.resolve<UpdateOrcamentoUseCase>(
    SERVICE_KEYS.UPDATE_ORCAMENTO_USE_CASE,
  );
  const aprovarUseCase = container.resolve<AprovarOrcamentoUseCase>(
    SERVICE_KEYS.APROVAR_ORCAMENTO_USE_CASE,
  );
  const rejeitarUseCase = container.resolve<RejeitarOrcamentoUseCase>(
  ...
  // Helper: Orçamentos por paciente
  const getOrcamentosByPatient = async (
    patientId: string,
  ): Promise<Orcamento[]> => {
    if (!patientId || !clinicId) return [];

    return queryClient.fetchQuery({
      queryKey: ["orcamentos", "patient", patientId, clinicId],
  ...
  const aprovarMutation = useMutation({
    mutationFn: async ({
      ...
      const { orcamento } = await aprovarUseCase.execute({
        orcamentoId,
        aprovadoPor,
      });
      return orcamento;
  ...
  const rejeitarMutation = useMutation({
    mutationFn: async ({
      ...
      const { orcamento } = await rejeitarUseCase.execute({
        orcamentoId,
        rejeitadoPor,
        motivo,
      });
