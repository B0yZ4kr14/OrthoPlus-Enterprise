...
export class ListOrcamentosUseCase {
  ...
  async execute(input: ListOrcamentosInput): Promise<ListOrcamentosOutput> {
    ...
    if (input.patientId) {
      orcamentos = await this.orcamentoRepository.findByPatientId(
        input.patientId,
        input.clinicId,
      );
    } else if (input.status) {
      orcamentos = await this.orcamentoRepository.findByStatus(
        input.clinicId,
        input.status,
      );
    } else {
      orcamentos = await this.orcamentoRepository.findByClinicId(
        input.clinicId,
      );
    }
