...
import type { CryptoTransaction, CryptoStats, ComparisonMethod, SavingsData, PieDataItem } from "./types";
...
export function useCryptoComparative({ transactions }: UseCryptoComparativeProps) {
  ...
  const avgSavingsPercentage = useMemo(() => {
    return savingsData.reduce((sum, item) => sum + item.savingsPercentage, 0) / savingsData.length;
  }, [savingsData]);
