...
interface CascadeAlertWizardProps {
  onSubmit: (cascadeAlerts: unknown[]) => void;
  onCancel: () => void;
}

const AVAILABLE_COINS = ["BTC", "ETH", "USDT", "BNB", "USDC"];

export function CascadeAlertWizard({
  onSubmit,
  onCancel,
}: CascadeAlertWizardProps) {
  ...
  const updateLevel = (
    id: string,
    field: "target_rate_brl" | "conversion_percentage",
    value: string,
  ) => {
  ...
  const validateAndSubmit = () => {
    ...
    const invalidLevels = levels.filter(
      (l) => !l.target_rate_brl || parseFloat(l.target_rate_brl) <= 0,
    );
    ...
    for (let i = 1; i < sortedLevels.length; i++) {
      ...
      if (alertType === "BELOW" && curr >= prev) {
        toast.error(
          "Para alertas BELOW, cada nível deve ter taxa menor que o anterior",
        );
        return;
      }
      if (alertType === "ABOVE" && curr <= prev) {
        toast.error(
          "Para alertas ABOVE, cada nível deve ter taxa maior que o anterior",
        );
        return;
      }
    }
    ...
    const totalPercentage = levels.reduce(
      (sum, l) => sum + parseFloat(l.conversion_percentage),
      0,
    );
  ...
  const totalPercentage = levels.reduce(
    (sum, l) => parseFloat(l.conversion_percentage || "0"),
    0,
  );
    ...
    <div className="space-y-6">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="BELOW">
                Venda Progressiva (Queda de Preço)
              </SelectItem>
              <SelectItem value="ABOVE">
                Compra Progressiva (Alta de Preço)
              </SelectItem>
            </SelectContent>
          </Select>
      ...
      <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <Input
                          ...
                          step="0.01"
                          placeholder={
                            alertType === "BELOW" ? "Ex: 320000" : "Ex: 360000"
                          }
        ...
        >
          ...
          Adicionar Nível{" "}
          {levels.length < 5 && `(${5 - levels.length} restantes)`}
        </Button>
      </div>
      ...
      {!isValidTotal && (
        <div className="p-3 border border-warning/50 bg-warning/10 rounded-lg">
          <p className="text-sm text-warning">
            ⚠️ A soma dos percentuais deve ser exatamente 100%. Ajuste os
            valores acima.
          </p>
        </div>
      )}
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="space-y-1">
            <p className="text-sm font-semibold">
              Dollar-Cost Averaging (DCA) Automatizado
            </p>
            <p className="text-xs text-muted-foreground">
              Esta estratégia converte sua posição em múltiplos níveis,
              reduzindo risco e otimizando pontos de saída ao invés de converter
              tudo de uma vez. Cada nível só dispara após o anterior.
            </p>
          </div>
        </div>
      </div>
