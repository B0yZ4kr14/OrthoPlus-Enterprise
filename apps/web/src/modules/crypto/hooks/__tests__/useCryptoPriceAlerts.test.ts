...
describe("useCryptoPriceAlerts", () => {
  ...
  it("should load alerts on mount", async () => {
    ...
    expect(mockGet).toHaveBeenCalledWith(
      "/crypto/price-alerts?clinic_id=clinic-1",
    );
  });
  ...
  it("should toggle alert and refetch", async () => {
    ...
    expect(mockPatch).toHaveBeenCalledWith("/crypto/price-alerts/a1", {
      is_active: false,
    });
    expect(toast.success).toHaveBeenCalledWith(
  ...
  it("should show activated message when toggling inactive alert", async () => {
    ...
    expect(mockPatch).toHaveBeenCalledWith("/crypto/price-alerts/a2", {
      is_active: true,
    });
