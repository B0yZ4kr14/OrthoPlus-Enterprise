...
import { Users, TrendingUp, Phone, Mail, Calendar, Plus, Target, Filter } from "lucide-react";
...
const mockLeads: Lead[] = [
  { id: "1", name: "Maria Silva", phone: "(11) 98765-4321", source: "Instagram", status: "novo", createdAt: "2026-05-10" },
  { id: "2", name: "João Santos", email: "joao@email.com", source: "Google Ads", status: "contatado", createdAt: "2026-05-09" },
  { id: "3", name: "Ana Costa", phone: "(11) 91234-5678", source: "Indicação", status: "agendado", createdAt: "2026-05-08" },
];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <StatsCard title="Total Leads" value={counts.total} icon={Users} variant="primary" description="cadastrados" />
        <StatsCard title="Novos" value={counts.novos} icon={Target} variant="default" description="aguardando contato" />
        <StatsCard title="Agendados" value={counts.agendados} icon={Calendar} variant="warning" description="consulta marcada" />
        <StatsCard title="Convertidos" value={counts.convertidos} icon={TrendingUp} variant="success" description="viraram pacientes" />
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2">
          {["all", "novo", "contatado", "agendado", "convertido", "perdido"].map((s) => (
            <Badge
              key={s}
      ...
      <Tabs defaultValue="funil" className="w-full">
        <TabsList className="grid w-full grid-cols-3 bg-muted/30 backdrop-blur-sm border border-border/50 rounded-xl p-1 max-w-md">
          <TabsTrigger value="funil" className="gap-2 rounded-lg data-[state=active]:bg-card data-[state=active]:shadow-sm data-[state=active]:text-foreground">
            <Filter className="h-4 w-4" />
            Funil
          </TabsTrigger>
          <TabsTrigger value="leads" className="gap-2 rounded-lg data-[state=active]:bg-card data-[state=active]:shadow-sm data-[state=active]:text-foreground">
            <Users className="h-4 w-4" />
            Leads
          </TabsTrigger>
          <TabsTrigger value="relatorios" className="gap-2 rounded-lg data-[state=active]:bg-card data-[state=active]:shadow-sm data-[state=active]:text-foreground">
          ...
          <Card className="glass-card overflow-hidden">
            ...
            {filtered.length === 0 ? (
              ...
              <div className="divide-y divide-border/50">
                {filtered.map((lead) => (
                  <div key={lead.id} className="p-4 hover:bg-muted/30 transition-all duration-200 group">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-semibold text-foreground group-hover:text-interactive transition-colors">{lead.name}</p>
                      ...
                      <Badge className={statusConfig[lead.status]?.color || ""}>{statusConfig[lead.status]?.label || lead.status}</Badge>
                    </div>
                  </div>
                ))}
              </div>
