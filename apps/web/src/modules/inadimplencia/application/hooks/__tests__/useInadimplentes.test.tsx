...
describe("useInadimplentes", () => {
  ...
  it("should show loading state and fetch inadimplentes on mount", async () => {
    mockGet.mockImplementation((url: string) => {
      ...
      if (url === "/campanhas-inadimplencia")
        return Promise.resolve([mockCampanha]);
    ...
    const inadimplentesList = result.current.inadimplentes as Array<
      Record<string, any>
    >;
    expect(inadimplentesList[0].id).toBe("ind-1");
    expect(result.current.campanhas).toHaveLength(1);
    const campanhasList = result.current.campanhas as Array<
      Record<string, any>
    >;
  ...
  it("should show toast.error when iniciarCobranca fails", async () => {
    ...
    await act(async () => {
      result.current.iniciarCobranca({
        inadimplenteId: "ind-1",
        tipo: "WHATSAPP",
      });
    });
    ...
    mockGet.mockImplementation((url: string) => {
      if (url === "/inadimplentes") {
        return new Promise((resolve) => {
          resolveInadimplentes = resolve;
        });
      }
      if (url === "/campanhas-inadimplencia") {
        return new Promise((resolve) => {
          resolveCampanhas = resolve;
        });
      }
      return Promise.resolve([]);
    });
  ...
  it("should return multiple inadimplentes and campanhas", async () => {
    mockGet.mockImplementation((url: string) => {
      if (url === "/inadimplentes")
        return Promise.resolve([mockInadimplente, mockInadimplente2]);
      if (url === "/campanhas-inadimplencia")
        return Promise.resolve([mockCampanha]);
      return Promise.resolve([]);
    });
