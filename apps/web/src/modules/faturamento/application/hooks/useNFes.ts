...
export function useNFes() {
  ...
  const { data: nfes = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<{ nfes: NFe[] }>("/faturamento/nfes")
  ...
  const createNFe = useMutation({
    mutationFn: async (data: CreateNFeData) => {
      const response = await apiClient.post<{ data: NFe }>("/faturamento/nfes", data)
  ...
  const authorizeNFe = useMutation({
    mutationFn: async ({ id, protocolo, xml }: { id: string; protocolo: string; xml: string }) => {
      await apiClient.post(`/faturamento/nfes/${id}/autorizar`, { protocolo, xml })
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["faturamento-nfes"] })
