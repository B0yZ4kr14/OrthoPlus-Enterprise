...
describe("useProcedimentosStore", () => {
  ...
  it("should initialize with mock data when localStorage is empty", async () => {
    ...
    await waitFor(() => expect(result.current.procedimentos.length).toBeGreaterThan(0))
  ...
  it("should load from localStorage when data exists", async () => {
    ...
    localStorage.setItem("ortho-plus-procedimentos", JSON.stringify(customData))
  ...
  it("should add a procedimento", async () => {
    ...
    const added = result.current.procedimentos[result.current.procedimentos.length - 1]
  ...
  it("should update a procedimento", async () => {
    ...
    act(() => {
      result.current.atualizarProcedimento("1", { nome: "Limpeza Atualizada", valor: 200 })
    })
  ...
  it("should not affect other procedimentos on update", async () => {
    ...
    const originalNome = result.current.procedimentos.find((p) => p.id === "2")?.nome

    act(() => {
      result.current.atualizarProcedimento("1", { nome: "Alterado" })
    })

    expect(result.current.procedimentos.find((p) => p.id === "2")?.nome).toBe(originalNome)
  })
  ...
  it("should delete a procedimento", async () => {
    ...
    expect(result.current.procedimentos.find((p) => p.id === "1")).toBeUndefined()
  })
  ...
  it("should filter procedimentos by categoria", async () => {
    ...
    expect(clinicaGeral.every((p) => p.categoria === "Clínica Geral")).toBe(true)
  ...
  it("should persist changes to localStorage", async () => {
    ...
    const stored = JSON.parse(localStorage.getItem("ortho-plus-procedimentos") || "[]")
