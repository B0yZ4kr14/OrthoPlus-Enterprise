...
export default function TabelaPrecosManager() {
  ...
  const [tipoTabela, setTipoTabela] = useState<"PARTICULAR" | "CONVENIO">(
    "PARTICULAR",
  );
  ...
  const { data: precos = [], refetch: refetchPrecos } = useQuery({
    ...
    queryFn: async () => {
      ...
      return apiClient.get<ProcedimentoPreco[]>(
        `/procedimentos/precos?tabela_id=${selectedTabela}`,
      );
    },
  ...
  const { data: templates = [] } = useQuery({
    ...
    queryFn: async () =>
      apiClient.get<Array<{ id: string; nome: string }>>(
  ...
  const handleCreateTabela = async () => {
    ...
    try {
      await apiClient.post("/procedimentos/tabelas", {
        nome: nomeTabela,
        tipo: tipoTabela,
  ...
  const handleUpdateTabela = async () => {
    ...
    setIsSubmitting(true);
    try {
      await apiClient.patch(`/procedimentos/tabelas/${editingTabela.id}`, {
        nome: nomeTabela,
        tipo: tipoTabela,
        is_default: isDefault,
      });
  ...
  const handleCreatePreco = async () => {
    if (!selectedTabela || !precoTemplate || !precoValor) {
        ...
        <TableBody>
          {tabelas.map((t: TabelaPreco) => (
            ...
            >
              ...
              <TableCell>
                <Badge
                  variant={t.tipo === "PARTICULAR" ? "default" : "secondary"}
                >
                  {t.tipo}
                </Badge>
              </TableCell>
        ...
        <div className="rounded-lg border p-4">
          <div className="mb-4 flex items-center justify-between">
            <h3 className="text-lg font-semibold">
              Preços —{" "}
              {tabelas.find((t: TabelaPreco) => t.id === selectedTabela)?.nome}
            </h3>
            <div className="flex gap-2">
              <Dialog
                open={isReajusteDialogOpen}
                onOpenChange={setIsReajusteDialogOpen}
              >
                <DialogTrigger asChild>
                  <Button variant="outline" size="sm">
                    <Percent className="mr-1 h-4 w-4" /> Reajuste
                  </Button>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Reajuste em Lote</DialogTitle>
                  </DialogHeader>
              ...
              <Dialog
                open={isPrecoDialogOpen}
                onOpenChange={setIsPrecoDialogOpen}
              >
                <DialogTrigger asChild>
                  <Button size="sm">
                ...
                <DialogContent>
                  ...
                  <div className="space-y-3">
                    <select
                      value={precoTemplate}
                      onChange={(e) => setPrecoTemplate(e.target.value)}
          ...
          <Table>
            ...
            <TableBody>
              {precos.map((p: ProcedimentoPreco) => (
                <TableRow key={p.id}>
                  <TableCell>
                    {p.procedimento_template?.nome ||
                      p.procedimento_template_id}
                  </TableCell>
