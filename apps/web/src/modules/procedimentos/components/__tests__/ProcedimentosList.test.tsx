...
vi.mock("@/components/shared/ActionButtons", () => ({
  ActionButtons: ({ onView, onEdit, onDelete }: any) => (
    <div>
      <button onClick={onView} title="Visualizar">Ver</button>
      <button onClick={onEdit} title="Editar">Edit</button>
      <button onClick={onDelete} title="Excluir">Del</button>
    </div>
  ),
}))
...
describe("ProcedimentosList", () => {
  ...
  it("should render empty state when no procedimentos", () => {
    ...
    expect(
      screen.getByText("Nenhum procedimento encontrado com os filtros selecionados."),
    ).toBeTruthy()
  })
  ...
  it("should filter procedimentos by categoria", () => {
    ...
    const categoriaSelect = selects[0].querySelector("select") as HTMLSelectElement
  ...
  it("should filter procedimentos by status", () => {
    ...
    const selects = screen.getAllByTestId("select")
    const statusSelect = selects[1].querySelector("select") as HTMLSelectElement
