...
describe("useFuncionariosStore", () => {
  ...
  it("should delete a funcionario", async () => {
    ...
    expect(
      result.current.funcionarios.find((f) => f.id === "1"),
    ).toBeUndefined();
    expect(toast.success).toHaveBeenCalledWith(
  ...
  it("should filter funcionarios by cargo", async () => {
    ...
    const admins = result.current.filterFuncionarios({
      cargo: "Administrador",
    });
  ...
  it("should return true when funcionario has the requested permission", async () => {
    ...
    const hasPermission = result.current.verificarPermissao(
      "1",
      "pacientes",
      "visualizar",
    );
    ...
    const hasEdit = result.current.verificarPermissao(
      "1",
      "pacientes",
      "editar",
    );
  ...
  it("should return false when funcionario does not have the requested permission", async () => {
    ...
    const noPermission = result.current.verificarPermissao(
      "1",
      "configuracoes",
      "excluir",
    );
  ...
  it("should return false for non-existent funcionario", async () => {
    ...
    const nonExistent = result.current.verificarPermissao(
      "999",
      "pacientes",
      "visualizar",
    );
  ...
  it("should return false for non-existent modulo", async () => {
    ...
    const noModulo = result.current.verificarPermissao(
      "1",
      "inexistente",
      "visualizar",
    );
  ...
  it("should persist changes to localStorage", async () => {
    ...
    const stored = JSON.parse(
      localStorage.getItem("orthoplus_funcionarios") || "[]",
    );
