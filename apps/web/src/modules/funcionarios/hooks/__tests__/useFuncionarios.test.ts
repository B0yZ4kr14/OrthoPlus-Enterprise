...
describe("useFuncionarios", () => {
  ...
  it("should show CPF duplicate error when error code is 23505", async () => {
    ...
    expect(toast.error).toHaveBeenCalledWith("CPF já cadastrado para esta clínica")
  })
  ...
  it("should update a funcionario and update state", async () => {
    mockGet.mockResolvedValueOnce([mockApiRow])
    mockPatch.mockResolvedValueOnce({ ...mockApiRow, nome: "João Silva Atualizado" })

    const { result } = renderHook(() => useFuncionarios())
    await waitFor(() => expect(result.current.loading).toBe(false))

    const updated = { ...result.current.funcionarios[0], nome: "João Silva Atualizado" }

    await act(async () => {
      await result.current.updateFuncionario("f1", updated)
    })

    expect(mockPatch).toHaveBeenCalledWith("/funcionarios/f1", expect.any(Object))
  ...
  it("should filter funcionarios by search, status and cargo", async () => {
    ...
    const filteredBySearch = result.current.filterFuncionarios({ search: "Maria" })
    expect(filteredBySearch).toHaveLength(1)
    expect(filteredBySearch[0].nome).toBe("Maria Souza")

    const filteredByStatus = result.current.filterFuncionarios({ status: "Ativo" })
    expect(filteredByStatus).toHaveLength(1)
    expect(filteredByStatus[0].nome).toBe("João Silva")

    const filteredByCargo = result.current.filterFuncionarios({ cargo: "Recepcionista" })
  ...
  it("should filter funcionarios by CPF and email search", async () => {
    ...
    const byCpf = result.current.filterFuncionarios({ search: "222.333.444-55" })
    expect(byCpf).toHaveLength(1)
    expect(byCpf[0].nome).toBe("Maria Souza")

    const byEmail = result.current.filterFuncionarios({ search: "joao@clinica.com" })
