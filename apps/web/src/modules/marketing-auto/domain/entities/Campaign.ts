...
export class Campaign {
  ...
  get targetSegment(): TargetSegment | undefined {
  ...
  getTotalSent(): number {
    return this.props.metrics?.totalSent || 0;
  }
  getTotalDelivered(): number {
    return this.props.metrics?.totalDelivered || 0;
  }
  getTotalOpened(): number {
    return this.props.metrics?.totalOpened || 0;
  }
  getTotalClicked(): number {
    return this.props.metrics?.totalClicked || 0;
  }
  getTotalConverted(): number {
    return this.props.metrics?.totalConverted || 0;
  }
  getTotalErrors(): number {
    return this.props.metrics?.totalErrors || 0;
  }
  ...
  updateMetrics(metrics: Partial<CampaignMetrics>): void {
    this.props.metrics = {
      ...
      totalDelivered:
        metrics.totalDelivered ?? this.props.metrics?.totalDelivered ?? 0,
      totalOpened: metrics.totalOpened ?? this.props.metrics?.totalOpened ?? 0,
      totalClicked:
        metrics.totalClicked ?? this.props.metrics?.totalClicked ?? 0,
      totalConverted:
        metrics.totalConverted ?? this.props.metrics?.totalConverted ?? 0,
  ...
  getErrorRate(): number {
    ...
    return (
      (this.props.metrics.totalErrors / this.props.metrics.totalSent) * 100
    );
  }
