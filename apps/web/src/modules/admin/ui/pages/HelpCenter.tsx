...
export default function HelpCenter() {
  ...
  const [selectedModule, setSelectedModule] = useState<OdontoTooltip | null>(
    null,
  );
                      ...
                      <CardHeader className="p-4">
                        <div className="space-y-2">
                          ...
                          <div className="flex flex-wrap gap-1">
                            ...
                            {module.riskLevel && (
                              <Badge
                                className={`text-xs ${getRiskColor(module.riskLevel)}`}
                              >
                                {module.riskLevel}
                              </Badge>
                            )}
          ...
          {selectedModule ? (
              ...
              <CardContent className="space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <ul className="space-y-2">
                      {selectedModule.indications.map((indication, index) => (
                        <li
                          key={index}
                          className="text-sm text-muted-foreground pl-6 relative before:content-['✓'] before:absolute before:left-0 before:text-success before:font-bold"
                        >
                          {indication}
                        </li>
                      ))}
                ...
                {selectedModule.contraindications.length > 0 && (
                  <div className="space-y-3">
                    ...
                    <ul className="space-y-2">
                      {selectedModule.contraindications.map(
                        (contraindication, index) => (
                          <li
                            key={index}
                            className="text-sm text-muted-foreground pl-6 relative before:content-['⚠'] before:absolute before:left-0 before:text-destructive before:font-bold"
                          >
                            {contraindication}
                          </li>
                        ),
                  ...
                  <div className="space-y-3">
                    ...
                    <ul className="space-y-2">
                      {selectedModule.bestPractices.map((practice, index) => (
                        <li
                          key={index}
                          className="text-sm text-muted-foreground pl-6 relative before:content-['→'] before:absolute before:left-0 before:text-primary before:font-bold"
                        >
                          {practice}
                        </li>
                      ))}
            ...
            <Card className="h-full flex items-center justify-center">
              <CardContent className="text-center py-12">
                ...
                <p className="text-muted-foreground">
                  Escolha um módulo na lista ao lado para ver a documentação
                  completa
                </p>
              </CardContent>
            </Card>
