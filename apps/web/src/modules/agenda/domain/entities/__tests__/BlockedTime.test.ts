...
describe("BlockedTime", () => {
  describe("constructor validation", () => {
    ...
    it("should throw when startDatetime is missing", () => {
      expect(
        () => new BlockedTime(makeProps({ startDatetime: undefined as unknown as Date })),
      ).toThrow("Data/hora de início é obrigatória")
    })

    it("should throw when endDatetime is missing", () => {
      expect(
        () => new BlockedTime(makeProps({ endDatetime: undefined as unknown as Date })),
    ...
    it("should throw when start equals end", () => {
      ...
      expect(() => new BlockedTime(makeProps({ startDatetime: d, endDatetime: d }))).toThrow(
        "Data/hora de início deve ser antes da data/hora de fim",
      )
    })
  ...
  describe("overlaps", () => {
    it("should return true when ranges overlap", () => {
      const bt = new BlockedTime(makeProps())
      expect(bt.overlaps(new Date("2024-01-01T11:00:00Z"), new Date("2024-01-01T13:00:00Z"))).toBe(
        true,
      )
    })
    ...
    it("should return false when ranges do not overlap", () => {
      const bt = new BlockedTime(makeProps())
      expect(bt.overlaps(new Date("2024-01-01T12:00:00Z"), new Date("2024-01-01T14:00:00Z"))).toBe(
        false,
      )
    })
  })
