...
describe("Appointment", () => {
  ...
  describe("constructor validation", () => {
    ...
    it("should throw when scheduledDatetime is missing", () => {
      expect(
        () =>
          new Appointment(
            makeProps({ scheduledDatetime: undefined as unknown as Date }),
          ),
  ...
  describe("computed properties", () => {
    it("should calculate endDatetime correctly", () => {
      const apt = new Appointment(
        makeProps({ scheduledDatetime: new Date("2024-01-01T10:00:00Z") }),
      );
    ...
    it("should allow cancellation when status is AGENDADO or CONFIRMADO", () => {
      expect(
        new Appointment(makeProps({ status: "AGENDADO" })).canBeCancelled,
      ).toBe(true);
      expect(
        new Appointment(makeProps({ status: "CONFIRMADO" })).canBeCancelled,
      ).toBe(true);
      expect(
        new Appointment(makeProps({ status: "REALIZADO" })).canBeCancelled,
      ).toBe(false);
      expect(
        new Appointment(makeProps({ status: "CANCELADO" })).canBeCancelled,
      ).toBe(false);
    });

    it("should allow reschedule when status is AGENDADO or CONFIRMADO", () => {
      expect(
        new Appointment(makeProps({ status: "AGENDADO" })).canBeRescheduled,
      ).toBe(true);
      expect(
        new Appointment(makeProps({ status: "CANCELADO" })).canBeRescheduled,
      ).toBe(false);
    });
  });
  ...
  describe("overlaps", () => {
    it("should return true when appointments overlap", () => {
      const apt1 = new Appointment(
        makeProps({
          scheduledDatetime: new Date("2024-01-01T10:00:00Z"),
          durationMinutes: 30,
        }),
      );
