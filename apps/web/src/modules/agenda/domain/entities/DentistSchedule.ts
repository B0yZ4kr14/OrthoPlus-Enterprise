...
export class DentistSchedule {
  ...
  private validate(): void {
    ...
    if (this.props.breakStart && this.props.breakEnd) {
      if (
        !this.isValidTimeFormat(this.props.breakStart) ||
        !this.isValidTimeFormat(this.props.breakEnd)
      ) {
        throw new Error("Horários de intervalo inválidos. Use formato HH:MM");
      }
      if (this.props.breakStart >= this.props.breakEnd) {
        throw new Error(
          "Início do intervalo deve ser antes do fim do intervalo",
        );
      }
      if (
        this.props.breakStart < this.props.startTime ||
        this.props.breakEnd > this.props.endTime
      ) {
  ...
  get breakStart(): string | undefined {
    return this.props.breakStart;
  }
  get breakEnd(): string | undefined {
    return this.props.breakEnd;
  }
  ...
  getAvailableSlots(slotDurationMinutes: number = 30): string[] {
    ...
    const breakStart = this.props.breakStart
      ? this.timeToMinutes(this.props.breakStart)
      : null;
    const breakEnd = this.props.breakEnd
      ? this.timeToMinutes(this.props.breakEnd)
      : null;

    for (
      let minutes = start;
      minutes + slotDurationMinutes <= end;
