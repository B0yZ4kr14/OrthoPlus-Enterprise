...
function TestConsumer({ filters }: { filters?: Parameters<typeof useAppointments>[0] }) {
  const {
    appointments,
    isLoading,
    createAppointment,
  ...
  return (
    <div>
      ...
      <div data-testid="appointments">{JSON.stringify(appointments.map((a) => ({ id: a.id, status: a.status })))}</div>
      <button
        data-testid="create"
        onClick={() =>
          createAppointment({
      ...
      <button data-testid="cancel" onClick={() => cancelAppointment({ appointmentId: "apt-1", reason: "Test" })}>
        Cancel
      </button>
...
describe("useAppointments", () => {
  ...
  it("should create an appointment via mutation", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("creating").textContent).toBe("idle"))
    expect(mockPost).toHaveBeenCalledWith("/agenda/appointments", expect.any(Object))
  })
  ...
  it("should cancel an appointment via mutation", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("cancelling").textContent).toBe("idle"))
    expect(mockPatch).toHaveBeenCalledWith("/agenda/appointments/apt-1", expect.any(Object))
  })
})
