...
function TestConsumer() {
  ...
  return (
    <div>
      ...
      <div data-testid="selected-dentist">{agenda.selectedDentistId || "none"}</div>
      ...
      <button data-testid="set-date" onClick={() => agenda.setCurrentDate(new Date("2024-06-15T00:00:00Z"))}>
        Set Date
      </button>
      <button data-testid="set-view" onClick={() => agenda.setViewMode("day")}>
        Day View
      </button>
      <button data-testid="set-dentist" onClick={() => agenda.setSelectedDentistId("dentist-1")}>
        Select Dentist
      </button>
    </div>
  )
...
describe("AgendaContext", () => {
  ...
  it("should compute week start and end from current date", () => {
    ...
    expect(new Date(weekEnd).getTime() - new Date(weekStart).getTime()).toBeGreaterThan(6 * 24 * 60 * 60 * 1000)
  })
  ...
  it("should go to today", () => {
    ...
    expect(Math.abs(new Date(after).getTime() - new Date(before).getTime())).toBeLessThan(24 * 60 * 60 * 1000)
  })

  it("should go to next week", () => {
    render(<TestConsumer />, { wrapper: Wrapper })
    const before = new Date(screen.getByTestId("week-start").textContent!).getTime()
    act(() => {
      screen.getByTestId("next-week").click()
    })
    const after = new Date(screen.getByTestId("week-start").textContent!).getTime()
  ...
  it("should set selected dentist", () => {
    ...
    expect(screen.getByTestId("selected-dentist").textContent).toBe("dentist-1")
  })
  ...
  it("should throw when useAgenda is used outside provider", () => {
    ...
    expect(() => render(<BadConsumer />)).toThrow("useAgenda must be used within AgendaProvider")
