...
describe("CreateAppointmentUseCase", () => {
  ...
  it("should create and save an appointment when no conflicts exist", async () => {
    mockBlockedTimeRepo.findByDentistAndDateRange = vi
      .fn()
      .mockResolvedValue([]);
    mockAppointmentRepo.findConflicts = vi.fn().mockResolvedValue([]);
    mockAppointmentRepo.save = vi
      .fn()
  ...
  it("should throw when dentist has blocked time in the period", async () => {
    mockBlockedTimeRepo.findByDentistAndDateRange = vi
      .fn()
      .mockResolvedValue([{ id: "bt-1" }] as any);
    ...
    await expect(useCase.execute(input)).rejects.toThrow(
      "Horário bloqueado para este dentista",
    );
  ...
  it("should throw when there is an appointment conflict", async () => {
    mockBlockedTimeRepo.findByDentistAndDateRange = vi
      .fn()
      .mockResolvedValue([]);
    mockAppointmentRepo.findConflicts = vi
      .fn()
      .mockResolvedValue([{ id: "apt-2" }] as any);
  ...
  it("should pass correct endDatetime to findConflicts", async () => {
    ...
    const conflictCall = (mockAppointmentRepo.findConflicts as any).mock
      .calls[0];
  ...
  it("should pass correct date range to blockedTimeRepository", async () => {
    ...
    const blockedCall = (mockBlockedTimeRepo.findByDentistAndDateRange as any)
      .mock.calls[0];
