...
function createMockAppointment(status: "AGENDADO" | "CONFIRMADO" | "REALIZADO" | "CANCELADO"): Appointment {
...
describe("CancelAppointmentUseCase", () => {
  ...
  it("should cancel an AGENDADO appointment and update repository", async () => {
    ...
    mockRepo.update = vi.fn().mockImplementation((a: Appointment) => Promise.resolve(a))

    const useCase = new CancelAppointmentUseCase(mockRepo)
    const result = await useCase.execute({ appointmentId: "apt-1", reason: "Motivo do cancelamento" })
  ...
  it("should throw when appointment is not found", async () => {
    ...
    const useCase = new CancelAppointmentUseCase(mockRepo)
    await expect(useCase.execute({ appointmentId: "missing-id" })).rejects.toThrow(
