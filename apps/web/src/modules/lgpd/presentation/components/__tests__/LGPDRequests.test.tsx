...
vi.mock("@orthoplus/core-ui/button", () => ({
  Button: ({ children, size }: any) => (
    <button data-testid="button" data-size={size}>{children}</button>
  ),
}))
...
describe("LGPDRequests", () => {
  ...
  it("should render card title", () => {
    render(<LGPDRequests />)
    expect(screen.getByTestId("card-title").textContent).toBe("Solicitações LGPD")
  })
  ...
  it("should render patient names and dates", () => {
    ...
    expect(screen.getByText((content) => content.includes("João Silva"))).toBeTruthy()
    expect(screen.getByText((content) => content.includes("Maria Santos"))).toBeTruthy()
    expect(screen.getByText((content) => content.includes("Pedro Costa"))).toBeTruthy()
  })
