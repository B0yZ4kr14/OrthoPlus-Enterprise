...
export const OdontogramaHistory = ({
  ...
  const handleRestore = (entry: OdontogramaHistoryEntry) => {
    if (
      confirm(
        `Deseja restaurar o odontograma para ${new Date(entry.timestamp).toLocaleString("pt-BR")}?`,
      )
    ) {
      onRestore(entry.id);
        ...
        <CardContent className="py-12">
          <div className="text-center text-muted-foreground">
            ...
            <p className="text-sm mt-1">
              As alterações no odontograma serão registradas automaticamente
            </p>
          </div>
        </CardContent>
      </Card>
            ...
            {history.map((entry, index) => (
              <div
                ...
                className={`border rounded-lg p-4 hover:bg-accent/50 transition-colors ${
                  selectedForComparison === entry.id
                    ? "ring-2 ring-primary"
                    : ""
                }`}
              >
