...
export function EvolucoesTimeline({ prontuarioId }: EvolucoesTimelineProps) {
  ...
  const { evolucoes: evolucoesData, isLoading } = useEvolucoes(
    prontuarioId,
    clinicId || "",
  );
            ...
            <div className="text-center text-muted-foreground">
              ...
              {searchTerm || filterTipo !== "TODOS" ? (
                ...
                <p className="text-sm mt-1">
                  Registre a primeira evolução do tratamento
                </p>
              )}
            </div>
          </CardContent>
          ...
          <div className="space-y-4">
            {filteredEvolucoes.map((evolucao, index) => (
              <div key={evolucao.id} className="relative pl-12">
                ...
                <div
                  className={`absolute left-3 top-3 w-5 h-5 rounded-full ${getTipoColor(evolucao.tipo)} border-4 border-background`}
                />
                    ...
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-muted-foreground flex items-center gap-1">
                          ...
                          {format(
                            new Date(evolucao.data_evolucao),
