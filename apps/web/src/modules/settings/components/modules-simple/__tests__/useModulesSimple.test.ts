...
describe("useModulesSimple", () => {
  ...
  it("should toggle module activation successfully", async () => {
    ...
    mockPost.mockResolvedValueOnce({ modules: [{ ...mockModule, is_active: false }] })
    ...
    expect(mockPost).toHaveBeenCalledWith("/modules/toggle", { module_key: "PACIENTES" })
    expect(toast.success).toHaveBeenCalledWith("Módulo atualizado com sucesso!")
  })

  it("should show cascade activation message when dependencies are activated", async () => {
    mockPost.mockResolvedValueOnce({ modules: [mockModule] })
    mockPost.mockResolvedValueOnce({ cascade_activated: 2, message: "2 módulos dependentes ativados" })
    mockPost.mockResolvedValueOnce({ modules: [mockModule] })
    ...
    expect(toast.success).toHaveBeenCalledWith("2 módulos dependentes ativados")
  })
  ...
  it("should show toast.error when fetching modules fails", async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith("Erro ao carregar módulos", expect.any(Error))
  })
  ...
  it("should show toast.error when toggle fails", async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith("Erro ao alternar módulo", expect.any(Error), { moduleKey: "PACIENTES" })
  })
  ...
  it("should group modules by category", async () => {
    ...
    expect(result.current.groupedModules["Atendimento Clínico"]).toHaveLength(1)
  ...
  it("should load roadmap successfully", async () => {
    mockPost.mockResolvedValueOnce({ modules: [mockModule] })
    const roadmapData = { recommendation: ["PACIENTES"], clinic_profile: { size: "small" } }
    ...
    expect(result.current.showRoadmap).toBe(true)
    expect(toast.success).toHaveBeenCalledWith("Roadmap de adoção gerado com sucesso!")
  })
  ...
  it("should show toast.error when loading roadmap fails", async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith("Erro ao gerar roadmap", expect.any(Error))
  })
  ...
  it("should activate modules by phase name matching", async () => {
    ...
    mockPost.mockResolvedValueOnce({ cascade_activated: 0 })
    mockPost.mockResolvedValueOnce({ modules: [{ ...inactiveModule, is_active: true }] })
    ...
    expect(mockPost).toHaveBeenCalledWith("/modules/toggle", { module_key: "PACIENTES" })
    expect(toast.success).toHaveBeenCalledWith("1 módulo(s) ativado(s) com sucesso!")
  })
  ...
  it("should activate modules via wizard", async () => {
    const inactiveModule = { ...mockModule, module_key: "FINANCEIRO", name: "Financeiro", is_active: false }
    mockPost.mockResolvedValueOnce({ modules: [mockModule, inactiveModule] })
    mockPost.mockResolvedValueOnce({ cascade_activated: 0 })
    mockPost.mockResolvedValueOnce({ modules: [mockModule, { ...inactiveModule, is_active: true }] })
    ...
    expect(mockPost).toHaveBeenCalledWith("/modules/toggle", { module_key: "FINANCEIRO" })
  })
})
