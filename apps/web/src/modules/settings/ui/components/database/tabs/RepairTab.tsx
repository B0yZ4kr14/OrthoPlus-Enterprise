...
const ENGINE_REPAIR_TOOLS: Record<
  string,
  { name: string; desc: string; cmd: string }[]
> = {
  PostgreSQL: [
    {
      name: "VACUUM FULL",
      desc: "Compacta e recupera espaço",
      cmd: "VACUUM FULL;",
    },
    { name: "ANALYZE", desc: "Atualiza estatísticas", cmd: "ANALYZE;" },
    {
      name: "REINDEX DATABASE",
      desc: "Reconstrói índices",
      cmd: "REINDEX DATABASE orthoplus;",
    },
    {
      name: "pg_checksums",
      desc: "Verifica checksums das páginas",
      cmd: "pg_checksums --check",
    },
  ],
  Firebird: [
    {
      name: "gfix -sweep",
      desc: "Remove versões antigas de registros",
      cmd: "gfix -sweep orthoplus.fdb",
    },
    {
      name: "gfix -validate",
      desc: "Valida estrutura do banco",
      cmd: "gfix -validate -full orthoplus.fdb",
    },
    {
      name: "gfix -mend",
      desc: "Repara erros encontrados",
      cmd: "gfix -mend orthoplus.fdb",
    },
    {
      name: "gstat",
      desc: "Estatísticas do banco",
      cmd: "gstat -h orthoplus.fdb",
    },
  ],
  MariaDB: [
    {
      name: "OPTIMIZE TABLE",
      desc: "Desfragmenta tabelas",
      cmd: "OPTIMIZE TABLE nome;",
    },
    { name: "CHECK TABLE", desc: "Verifica erros", cmd: "CHECK TABLE nome;" },
    { name: "REPAIR TABLE", desc: "Repara erros", cmd: "REPAIR TABLE nome;" },
    {
      name: "ANALYZE TABLE",
      desc: "Atualiza estatísticas",
      cmd: "ANALYZE TABLE nome;",
    },
  ],
  SQLite: [
...
export function RepairTab({ selectedEngine, category }: RepairTabProps) {
  ...
  const tools =
    ENGINE_REPAIR_TOOLS[selectedEngine] || ENGINE_REPAIR_TOOLS.PostgreSQL;
  ...
  const handleRun = async (toolName: string) => {
    if (selectedEngine === "PostgreSQL") {
      try {
        ...
        showSuccess(
          `Operação ${toolName} executada com sucesso no PostgreSQL.`,
        );
      } catch (error) {
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Otimize e repare a base de dados desta categoria
        </p>
      </div>
      ...
      <div className="mt-8 space-y-4">
        <h4 className="text-sm font-medium text-foreground">
          Histórico de Conexão e Manutenção
        </h4>
