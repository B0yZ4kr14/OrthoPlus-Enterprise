...
> = {
  PostgreSQL: {
    ...
    whenToUse: [
      "Ambientes corporativos com alta concorrência",
      "Alta disponibilidade com replicação",
      "Dados JSON semi-estruturados (JSONB)",
      "Queries analíticas complexas",
    ],
    limitations: [
      "Configuração inicial mais complexa",
      "Consumo de memória maior",
      "Overhead para bancos pequenos",
    ],
    resources: [
      "JSON/JSONB nativo",
  ...
  SQLite: {
    ...
    whenToUse: [
      ...
      "Aplicações single-node",
      "Dispositivos com poucos recursos",
      "Banco embutido em app",
    ],
    limitations: [
      "Sem multi-user concorrente",
      "Sem servidor remoto nativo",
  ...
  MariaDB: {
    ...
    whenToUse: [
      "Migração de sistemas MySQL",
      "Workloads OLTP tradicionais",
      "Compatibilidade com legado",
      "Cluster com Galera",
    ],
    limitations: [
      "Menos nativo no ecossistema Node/Prisma",
      "JSON menos poderoso que PostgreSQL",
      "Extensões limitadas",
    ],
    resources: [
      "MySQL compatible",
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Selecione o SGBD que melhor atende esta categoria
        </p>
      </div>
      ...
      {currentDetails && (
        <Card className="border border-border bg-muted/20 mt-8">
          <CardContent className="p-6">
            ...
            <div>
              <h5 className="text-sm font-medium text-info mb-3">
                🚀 Recursos:
              </h5>
              <div className="flex flex-wrap gap-2">
                {currentDetails.resources.map((resource, idx) => (
                  <Badge
                    key={idx}
                    variant="outline"
                    className="border-interactive/30 bg-interactive/10 text-interactive"
                  >
                    {resource}
                  </Badge>
                ))}
