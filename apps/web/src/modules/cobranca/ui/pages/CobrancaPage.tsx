...
export default function CobrancaPage() {
  const { inadimplentes, stats, isLoading } = useInadimplencia();
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {new Intl.NumberFormat("pt-BR", {
                style: "currency",
                currency: "BRL",
              }).format(stats.totalEmAberto)}
            </div>
            <p className="text-xs text-muted-foreground">
              {stats.countTotal} faturas pendentes
            </p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-destructive">
              {new Intl.NumberFormat("pt-BR", {
                style: "currency",
                currency: "BRL",
              }).format(stats.totalVencido)}
            </div>
            <p className="text-xs text-muted-foreground">
              {stats.countVencidos} faturas vencidas
            </p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-warning">
              {new Intl.NumberFormat("pt-BR", {
                style: "currency",
                currency: "BRL",
              }).format(stats.totalAVencer)}
            </div>
            <p className="text-xs text-muted-foreground">
              {stats.countAVencer} faturas próximas
            </p>
          </CardContent>
        </Card>
      ...
      <Tabs defaultValue="inadimplentes" className="space-y-4">
                      ...
                      >
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center gap-2 mt-2">
                            <Badge
                              variant={
                                item.dias_atraso && item.dias_atraso > 30
                                  ? "destructive"
                                  : "warning"
                              }
                            >
                              Vencido há {item.dias_atraso || 0} dias
                            </Badge>
                            <Badge variant="outline">
                              {new Intl.NumberFormat("pt-BR", {
                                style: "currency",
                                currency: "BRL",
                              }).format(item.valor_pendente)}
                            </Badge>
                          </div>
                        </div>
        ...
        <TabsContent value="comunicacao" className="space-y-4">
          <Card>
            ...
            <CardContent>
              <p className="text-sm text-muted-foreground">
                Funcionalidade em desenvolvimento - Integração com WhatsApp
                Business API e e-mail
              </p>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="historico" className="space-y-4">
          <Card>
            ...
            <CardContent>
              <p className="text-sm text-muted-foreground">
                Funcionalidade em desenvolvimento - Audit log de comunicações
                enviadas
              </p>
            </CardContent>
          </Card>
