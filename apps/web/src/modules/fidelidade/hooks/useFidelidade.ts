...
export function useFidelidade() {
  ...
  const createRecompensa = async (recompensaData: unknown) => {
    if (!user || !selectedClinic) {
      toast.error("Usuário não autenticado");
      return;
    }
  ...
  const updateRecompensa = async (id: string, recompensaData: unknown) => {
    if (!user) {
      toast.error("Usuário não autenticado");
      return;
    }
  ...
  const createBadge = async (badgeData: unknown) => {
    if (!user || !selectedClinic) {
      toast.error("Usuário não autenticado");
      return;
    }
