...
describe("usePDV", () => {
  ...
  it("should throw error when clinicId is missing on abrirCaixa", async () => {
    ...
    await expect(result.current.abrirCaixa(500)).rejects.toThrow("Clinic ID required")
  ...
  it("should throw error when user is not authenticated on abrirCaixa", async () => {
    ...
    await expect(result.current.abrirCaixa(500)).rejects.toThrow("User not authenticated")
    expect(mockPost).not.toHaveBeenCalled()
  })
  ...
  it("should handle error on abrirCaixa", async () => {
    ...
    await expect(result.current.abrirCaixa(500)).rejects.toThrow("Server error")
  })
  ...
  it("should throw error when no caixa is open on fecharCaixa", async () => {
    ...
    await expect(result.current.fecharCaixa(600)).rejects.toThrow("No caixa open")
    expect(mockPost).not.toHaveBeenCalled()
  })
  ...
  it("should handle error on fecharCaixa", async () => {
    ...
    await expect(result.current.fecharCaixa(600)).rejects.toThrow("Close failed")
  })
  ...
  it("should create venda and reload vendas", async () => {
    ...
    const vendaData = { valor_total: 200, desconto: 0, status: "FINALIZADA" as const }
    const itens = [
      {
  ...
  it("should throw error when caixa is not open on criarVenda", async () => {
    ...
    await expect(
      result.current.criarVenda({}, [], []),
    ).rejects.toThrow("Caixa must be open")
    expect(mockPost).not.toHaveBeenCalled()
  })
  ...
  it("should handle error on criarVenda", async () => {
    ...
    await expect(result.current.criarVenda({}, [], [])).rejects.toThrow("Create failed")
  })
