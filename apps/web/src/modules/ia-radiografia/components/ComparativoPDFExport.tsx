...
export function ComparativoPDFExport({
  analise1,
  analise2,
  comparacao,
}: ComparativoPDFExportProps) {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      pdf.text(
        "Relatório Comparativo de Radiografias",
        pageWidth / 2,
        yPosition,
        { align: "center" },
      );
      ...
      pdf.text(
        `Período entre análises: ${comparacao.diasEntre} dias`,
        margin,
        yPosition,
      );
      yPosition += 6;
      ...
      // Precisão da IA
      const precisaoCor =
        comparacao.precisao.tendencia === "melhorou"
          ? [34, 197, 94]
          : comparacao.precisao.tendencia === "piorou"
            ? [239, 68, 68]
            : [107, 114, 128];
      ...
      pdf.setFontSize(14);
      pdf.setFont("helvetica", "bold");
      pdf.text("Comparação Visual das Radiografias", pageWidth / 2, yPosition, {
        align: "center",
      });
      yPosition += 12;
      ...
      yPosition += 4;
      pdf.text(
        `Problemas: ${analise1.problemas_detectados || 0}`,
        margin,
        yPosition,
      );
      yPosition += 4;
      pdf.text(
        `Precisão IA: ${analise1.confidence_score ? Math.round(analise1.confidence_score) : 0}%`,
        margin,
        yPosition,
      );
      yPosition += 8;
      ...
      yPosition += 4;
      pdf.text(
        `Problemas: ${analise2.problemas_detectados || 0}`,
        margin,
        yPosition,
      );
      yPosition += 4;
      pdf.text(
        `Precisão IA: ${analise2.confidence_score ? Math.round(analise2.confidence_score) : 0}%`,
        margin,
        yPosition,
      );
      yPosition += 8;
