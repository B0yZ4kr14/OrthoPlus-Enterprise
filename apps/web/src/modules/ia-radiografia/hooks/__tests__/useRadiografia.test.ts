...
const mockAnalise: AnaliseComplete = {
  ...
  resultado_ia: {
    problemas_detectados: [
      { tipo_problema: "CARIE", localizacao: "Dente 14", severidade: "MODERADA" } as any,
    ],
...
describe("useRadiografia", () => {
  ...
  it("should upload radiografia and reload data", async () => {
    ...
    await act(async () => {
      await result.current.uploadRadiografia("patient-1", undefined, "PANORAMICA", file)
    })
  ...
  it("should throw error when user is not authenticated on upload", async () => {
    ...
    await act(async () => {
      await expect(
        result.current.uploadRadiografia("patient-1", undefined, "PANORAMICA", file),
  ...
  it("should include prontuario_id in form data when provided", async () => {
    ...
    await act(async () => {
      await result.current.uploadRadiografia("patient-1", "pront-1", "PANORAMICA", file)
    })
