...
export default function EstoqueMovimentacoesPage() {
  ...
  const { produtos, fornecedores, movimentacoes, loading, addMovimentacao } =
    useEstoque();
  ...
  const movimentacoesEntrada = movimentacoes.filter(
    (m) => m.tipo === "ENTRADA" || m.tipo === "DEVOLUCAO",
  );
  const movimentacoesSaida = movimentacoes.filter(
    (m) => m.tipo === "SAIDA" || m.tipo === "PERDA",
  );
  ...
  const totalEntradas = movimentacoesEntrada.reduce(
    (sum, m) => sum + (m.valorTotal || 0),
    0,
  );
  const totalSaidas = movimentacoesSaida.reduce(
    (sum, m) => sum + (m.valorTotal || 0),
    0,
  );
    ...
    return (
      <div className="space-y-6">
        ...
        <LoadingState
          variant="spinner"
          size="lg"
          message="Carregando movimentações..."
        />
      </div>
    );
  }
