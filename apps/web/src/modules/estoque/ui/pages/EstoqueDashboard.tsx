...
export default function EstoqueDashboard() {
  const { produtos, requisicoes, movimentacoes, alertas, loading } =
    useEstoque();
  ...
  const metrics = useMemo(() => {
    ...
    const requisiçõesPendentes = requisicoes.filter(
      (r) => r.status === "PENDENTE",
    ).length;
    ...
    return produtos
      ...
      .sort(
        (a, b) =>
          ...
          b.quantidadeAtual / b.quantidadeMinima,
      )
      .slice(0, 10)
      .map((p) => ({
  ...
  const movimentacoesRecentes = useMemo(() => {
    ...
    const dadosPorDia: Record<
      string,
      { entradas: number; saidas: number; data: string }
    > = {};
    ...
    return (
      <div className="space-y-6">
        ...
        <LoadingState
          variant="spinner"
          size="lg"
          message="Carregando métricas do estoque..."
        />
      </div>
    );
  }
                  ...
                  <div className="text-right">
                    ...
                    {alerta.quantidadeSugerida && (
                      <p className="text-xs text-muted-foreground">
                        Sugerido: {alerta.quantidadeSugerida}
                      </p>
                    )}
                  </div>
                </div>
