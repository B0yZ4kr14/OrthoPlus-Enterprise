...
export default function EstoquePedidos() {
  ...
  const handleGerarPedidos = async () => {
    ...
    try {
      ...
      toast({
        title: "Sucesso",
        description: "Pedidos automáticos gerados com sucesso!",
      });
    } catch (error) {
      toast({
        title: "Erro",
        description: "Erro ao gerar pedidos automáticos",
        variant: "destructive",
      });
    } finally {
                        ...
                        <div className="flex justify-between items-start">
                          <div className="space-y-2">
                            ...
                            <div className="text-sm text-muted-foreground space-y-1">
                              <p>
                                Ponto de pedido: {config.pontoPedido} unidades
                              </p>
                              <p>
                                Quantidade de reposição:{" "}
                                {config.quantidadeReposicao} unidades
                              </p>
                              <p>
                                Prazo de entrega: {config.diasEntregaEstimados}{" "}
                                dias
                              </p>
                              <p>
                                Status:{" "}
                                {config.gerarAutomaticamente
                                  ? "Ativo"
                                  : "Inativo"}
                              </p>
                            </div>
