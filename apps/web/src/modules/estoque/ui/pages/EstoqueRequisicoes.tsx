...
export default function EstoqueRequisicoes() {
  ...
  const isAdmin =
    authUser?.app_role === "ADMIN" || authUser?.app_role === "ROOT";
  ...
  const handleRejeitar = async () => {
    ...
    try {
      await rejeitarRequisicao(
        rejectDialog.id,
        "Requisição rejeitada pelo gestor",
      );
  ...
  const requisicoesAprovadas = requisicoes.filter(
    (r) => r.status === "APROVADA" || r.status === "ENTREGUE",
  );
  const requisicoesRejeitadas = requisicoes.filter(
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza que deseja rejeitar esta requisição? O solicitante
              será notificado.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
