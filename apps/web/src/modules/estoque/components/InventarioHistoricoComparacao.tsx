...
export function InventarioHistoricoComparacao() {
  ...
  const filtrarInventariosPorPeriodo = () => {
    ...
    const filtrados = inventarios
      .filter(
        (inv) => new Date(inv.data) >= dataInicio && inv.status === "CONCLUIDO",
      )
  ...
  const dadosEvolucaoDivergencias = inventariosFiltrados.map((inv) => ({
    ...
    acuracidade: inv.totalItens
      ? (1 - (inv.divergenciasEncontradas || 0) / inv.totalItens) * 100
      : 0,
  }));
  ...
  const distribuicaoCriticidade = inventariosFiltrados.reduce(
    (acc, inv) => {
      const items = inventarioItems.filter(
        (item) => item.inventarioId === inv.id,
      );
      items.forEach((item) => {
  ...
  // Estatísticas gerais
  const totalDivergencias = inventariosFiltrados.reduce(
    (sum, inv) => sum + (inv.divergenciasEncontradas || 0),
    0,
  );
  const totalValorDivergencias = inventariosFiltrados.reduce(
  ...
  const acuracidadeMedia =
    inventariosFiltrados.length > 0
      ? inventariosFiltrados.reduce((sum, inv) => {
          const acuracidade = inv.totalItens
            ? (1 - (inv.divergenciasEncontradas || 0) / inv.totalItens) * 100
            : 100;
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold">
            Análise Histórica de Inventários
          </h2>
      ...
      <Card className="p-6">
        <h3 className="text-lg font-semibold mb-4">
          Evolução de Divergências e Acuracidade
        </h3>
      ...
      {produtosComPerdas.length > 0 && (
        <Card className="p-6">
          <h3 className="text-lg font-semibold mb-4">
            Detalhamento de Produtos com Perdas Recorrentes
          </h3>
