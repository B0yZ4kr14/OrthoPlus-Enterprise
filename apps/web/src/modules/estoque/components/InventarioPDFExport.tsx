...
export async function exportInventarioPDF(
  ...
  doc.text("Relatório de Divergências de Inventário", pageWidth / 2, 20, {
    align: "center",
  });
  ...
  const resumoData = [
    ...
    [
      ...
      inventario.divergenciasEncontradas?.toString() || "0",
    ],
    ["Valor Total das Divergências", `R$ ${totalValorDivergencias.toFixed(2)}`],
    [
      "Acuracidade",
      `${((1 - divergencias.length / items.length) * 100).toFixed(1)}%`,
    ],
  ];
  ...
  const alta = divergencias.filter(
    (d) => Math.abs(d.percentualDivergencia || 0) >= 20,
  ).length;
  const media = divergencias.filter((d) => {
    const perc = Math.abs(d.percentualDivergencia || 0);
    return perc >= 10 && perc < 20;
  }).length;
  const baixa = divergencias.filter(
    (d) => Math.abs(d.percentualDivergencia || 0) < 10,
  ).length;
  ...
  const criticidadeData = [
    [
      "Alta (≥20%)",
      alta.toString(),
      `${((alta / divergencias.length) * 100).toFixed(1)}%`,
    ],
    [
      "Média (10-20%)",
      media.toString(),
      `${((media / divergencias.length) * 100).toFixed(1)}%`,
    ],
    [
      "Baixa (<10%)",
      baixa.toString(),
      `${((baixa / divergencias.length) * 100).toFixed(1)}%`,
    ],
  ];
