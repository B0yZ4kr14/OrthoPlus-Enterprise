...
export function useEstoqueStore() {
  ...
  const [fornecedores, setFornecedores] = useLocalStorage<Fornecedor[]>(
    "estoque-fornecedores",
    MOCK_FORNECEDORES,
  );
  const [categorias, setCategorias] = useLocalStorage<Categoria[]>(
    "estoque-categorias",
    MOCK_CATEGORIAS,
  );
  const [requisicoes, setRequisicoes] = useLocalStorage<Requisicao[]>(
    "estoque-requisicoes",
    [],
  );
  const [movimentacoes, setMovimentacoes] = useLocalStorage<Movimentacao[]>(
    "estoque-movimentacoes",
    [],
  );
  const [alertas, setAlertas] = useLocalStorage<Alerta[]>(
    "estoque-alertas",
    [],
  );
  ...
  const updateFornecedor = (id: string, data: Partial<Fornecedor>) => {
    setFornecedores(
      fornecedores.map((f) => (f.id === id ? { ...f, ...data } : f)),
    );
  };
  ...
  const updateRequisicao = (id: string, data: Partial<Requisicao>) => {
    setRequisicoes(
      requisicoes.map((r) => (r.id === id ? { ...r, ...data } : r)),
    );
  };
