...
describe("useEstoqueStore", () => {
  ...
  it("should update a produto", () => {
    ...
    act(() => {
      result.current.updateProduto("1", {
  ...
  it("should clear read alerts", () => {
    ...
    act(() => {
      result.current.marcarAlertaComoLido(
        result.current.alertas[0]!.id as string,
      );
    });
  ...
  it("should add, update and delete categoria", () => {
    ...
    act(() => {
      result.current.updateCategoria(categoriaId, {
        nome: "Categoria Atualizada",
      });
    });

    expect(
      result.current.categorias.find((c) => c.id === categoriaId)?.nome,
    ).toBe("Categoria Atualizada");
    ...
    expect(result.current.categorias).toHaveLength(3);
    expect(
      result.current.categorias.find((c) => c.id === categoriaId),
    ).toBeUndefined();
  });
  ...
  it("should add, update and delete fornecedor", () => {
    ...
    act(() => {
      result.current.updateFornecedor(fornecedorId, {
  ...
  it("should add, aprovar and rejeitar requisicao", () => {
    ...
    const aprovada = result.current.requisicoes.find(
      (r) => r.id === requisicaoId,
    );
    ...
    const rejeitada = result.current.requisicoes.find(
      (r) => r.id === requisicaoId,
    );
  ...
  it("should persist movimentacoes to localStorage", () => {
    ...
    const stored: Movimentacao[] = JSON.parse(
      storage["estoque-movimentacoes"] || "[]",
    );
