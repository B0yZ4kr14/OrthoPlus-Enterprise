...
const authState: { user: Record<string, any> | null } = {
  user: { id: "user-1" },
};
...
function mockAllEndpoints(
  ...
  mockGet.mockImplementation((url: string) => {
    if (url === "/estoque/produtos")
      return Promise.resolve(opts.produtos ?? []);
    if (url === "/estoque/categorias")
      return Promise.resolve(opts.categorias ?? []);
    if (url === "/estoque/fornecedores")
      return Promise.resolve(opts.fornecedores ?? []);
    if (url === "/estoque/requisicoes")
      return Promise.resolve(opts.requisicoes ?? []);
    if (url === "/estoque/movimentacoes")
      return Promise.resolve(opts.movimentacoes ?? []);
    if (url === "/estoque/alertas") return Promise.resolve(opts.alertas ?? []);
    if (url === "/estoque/pedidos") return Promise.resolve(opts.pedidos ?? []);
    if (url === "/estoque/pedidos-itens")
      return Promise.resolve(opts.pedidosItens ?? []);
    if (url === "/estoque/pedidos-config")
      return Promise.resolve(opts.pedidosConfig ?? []);
...
describe("useEstoque", () => {
  ...
  it("should show toast.error on deleteProduto failure", async () => {
    ...
    await act(async () => {
      await expect(result.current.deleteProduto("p1")).rejects.toThrow(
        "Delete failed",
      );
    });
  ...
  it("should add a categoria", async () => {
    ...
    await act(async () => {
      await result.current.addCategoria({
        nome: "Nova Categoria",
        descricao: "Desc",
      });
    });

    expect(mockPost).toHaveBeenCalledWith("/estoque/categorias", {
      nome: "Nova Categoria",
      descricao: "Desc",
    });
  ...
  it("should update a categoria", async () => {
    ...
    expect(mockPatch).toHaveBeenCalledWith("/estoque/categorias/c1", {
      nome: "Cat Atualizada",
    });
  ...
  it("should get categoria by id", async () => {
    ...
    expect(result.current.getCategoriaById("c1")?.nome).toBe(
  ...
  it("should get fornecedor by id", async () => {
    ...
    expect(result.current.getFornecedorById("f1")?.nome).toBe(
      "Dental Supply Ltda",
    );
  ...
  it("should marcar alerta como lido", async () => {
    ...
    expect(mockPatch).toHaveBeenCalledWith("/estoque/alertas/a1", {
      lido: true,
    });
  });
  ...
  it("should show toast.error on marcarAlertaComoLido failure", async () => {
    ...
    await act(async () => {
      await expect(result.current.marcarAlertaComoLido("a1")).rejects.toThrow(
        "Update failed",
      );
    });
  ...
  it("should show toast.error on limparAlertasLidos failure", async () => {
    ...
    await act(async () => {
      await expect(result.current.limparAlertasLidos()).rejects.toThrow(
        "Delete failed",
      );
    });
  ...
  it("should update pedido status", async () => {
    ...
    expect(mockPatch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        status: "RECEBIDO",
        data_recebimento: expect.any(String),
      }),
    );
  ...
  it("should gerar pedidos automaticos", async () => {
    ...
    expect(mockPost).toHaveBeenCalledWith(
      "/estoque/pedidos/gerar-automaticos",
      {},
    );
    expect(toast.success).toHaveBeenCalledWith(
      "Pedidos automáticos gerados com sucesso",
    );
  });
