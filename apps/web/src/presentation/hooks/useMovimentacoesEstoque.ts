...
export function useMovimentacoesEstoque(produtoId?: string) {
  ...
  const registrarEntradaUseCase = useService<RegistrarEntradaUseCase>(
    SERVICE_KEYS.REGISTRAR_ENTRADA_USE_CASE,
  );
  const registrarSaidaUseCase = useService<RegistrarSaidaUseCase>(
    SERVICE_KEYS.REGISTRAR_SAIDA_USE_CASE,
  );
  const ajustarEstoqueUseCase = useService<AjustarEstoqueUseCase>(
