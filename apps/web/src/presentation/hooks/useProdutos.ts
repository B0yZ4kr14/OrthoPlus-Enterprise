...
export function useProdutos(clinicId: string, apenasAtivos: boolean = false) {
  ...
  const createProdutoUseCase = useService<CreateProdutoUseCase>(
    SERVICE_KEYS.CREATE_PRODUTO_USE_CASE,
  );
  const updateProdutoUseCase = useService<UpdateProdutoUseCase>(
    SERVICE_KEYS.UPDATE_PRODUTO_USE_CASE,
  );
  const getProdutoByIdUseCase = useService<GetProdutoByIdUseCase>(
    SERVICE_KEYS.GET_PRODUTO_BY_ID_USE_CASE,
  );
  const listProdutosUseCase = useService<ListProdutosByClinicUseCase>(
  ...
  const {
    ...
    queryFn: async () => {
      const result = await listProdutosUseCase.execute({
        clinicId,
        apenasAtivos,
      });
