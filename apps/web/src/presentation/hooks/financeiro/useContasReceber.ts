...
import {
  ContaReceber,
  StatusContaReceber,
} from "@/domain/entities/ContaReceber";
import type {
...
export function useContasReceber() {
  ...
  // Use Cases
  const createContaReceberUseCase =
    container.resolve<CreateContaReceberUseCase>(
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err
            : new Error("Erro ao carregar contas a receber"),
        );
      } finally {
        setLoading(false);
      }
  ...
  const totalAReceber = contasPendentes.reduce(
    (sum, c) => sum + c.calcularSaldoReceber(),
    0,
  );
  ...
  const totalRecebido = contasRecebidas.reduce(
    (sum, c) => sum + (c.valorPago || 0),
    0,
  );
