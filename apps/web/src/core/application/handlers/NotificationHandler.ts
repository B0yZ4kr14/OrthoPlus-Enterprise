...
export class NotificationHandler implements IEventHandler<DomainEvent> {
  private messageMap: Record<
    string,
    (event: Record<string, unknown>) => string
  > = {
    TransactionCreated: (e) =>
      `Transação criada: R$ ${(e.data as Record<string, unknown>).amount}`,
    TransactionPaid: (e) =>
      `Pagamento confirmado: R$ ${(e.data as Record<string, unknown>).amount}`,
    ProdutoEstoqueBaixo: (e) =>
      `⚠️ Estoque baixo: ${(e.data as Record<string, unknown>).produtoNome}`,
    LeadConverted: (e) =>
      `🎉 Lead convertido: ${(e.data as Record<string, unknown>).leadNome}`,
    AppointmentScheduled: (e) =>
      `Consulta agendada: ${(e.data as Record<string, unknown>).patientName}`,
  };
