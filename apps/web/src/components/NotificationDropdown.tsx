...
export const NotificationDropdown = () => {
  ...
  const handleNotificationClick = (notification: unknown) => {
    if (!(notification as { lida: boolean }).lida) {
      void markAsRead((notification as { id: string }).id);
    }
    if ((notification as { link_acao?: string }).link_acao) {
      navigate((notification as { link_acao: string }).link_acao);
    }
  };
      ...
      >
        ...
        <ScrollArea className="h-[400px]">
          {notifications.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-muted-foreground glass-card rounded-xl m-2">
