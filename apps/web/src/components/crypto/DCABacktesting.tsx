...
export function DCABacktesting() {
  ...
  const runBacktest = async () => {
    ...
    try {
      ...
      const backtestResults: BacktestResult[] = monthlyPrices.map(
        (item, index) => {
          // DCA: compra mensal
      ...
      setSummary({
        ...
        lumpSumReturn:
          ((lumpSumFinalValue - lumpSumTotalInvested) / lumpSumTotalInvested) *
          100,
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-semibold">
                          {summary.dcaTotalInvested.toLocaleString("pt-BR", {
                            style: "currency",
                            currency: "BRL",
                          })}
                        </span>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-semibold text-primary">
                          {summary.dcaFinalValue.toLocaleString("pt-BR", {
                            style: "currency",
                            currency: "BRL",
                          })}
                        </span>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span
                          className={`font-semibold ${summary.dcaReturn >= 0 ? "text-success" : "text-destructive"}`}
                        >
                          {summary.dcaReturn >= 0 ? "+" : ""}
                  ...
                  <CardContent className="p-4 space-y-3">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-semibold">
                          {summary.lumpSumTotalInvested.toLocaleString(
                            "pt-BR",
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-semibold text-secondary">
                          {summary.lumpSumFinalValue.toLocaleString("pt-BR", {
                            style: "currency",
                            currency: "BRL",
                          })}
                        </span>
                      ...
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Retorno:</span>
                        <span
                          className={`font-semibold ${summary.lumpSumReturn >= 0 ? "text-success" : "text-destructive"}`}
                        >
                  ...
                  {summary.dcaReturn > summary.lumpSumReturn ? (
                    <>
                      <span className="font-semibold text-success">
                        DCA foi{" "}
                        {(summary.dcaReturn - summary.lumpSumReturn).toFixed(2)}
                        % melhor
                      </span>{" "}
                      que investimento único neste período. O DCA reduz risco ao
                      distribuir compras ao longo do tempo, evitando timing de
                      mercado.
                    </>
                  ) : (
                    <>
                      <span className="font-semibold text-secondary">
                        Investimento único foi{" "}
                        {(summary.lumpSumReturn - summary.dcaReturn).toFixed(2)}
                        % melhor
                      </span>{" "}
                      que DCA neste período. Em mercados em alta constante, lump
                      sum tende a superar DCA por entrar com capital total no
                      início.
                    </>
                  )}
                </p>
