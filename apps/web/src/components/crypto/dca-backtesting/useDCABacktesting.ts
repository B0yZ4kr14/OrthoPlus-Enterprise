...
export function useDCABacktesting() {
  ...
  const calculateMonthlyPrices = useCallback(
    (prices: number[][], period: number, endDate: Date): MonthlyPrice[] => {
    ...
    (
      ...
      return {
        ...
        summary: {
          ...
          lumpSumReturn:
            ((lumpSumFinalValue - lumpSumTotalInvested) /
              lumpSumTotalInvested) *
            100,
  ...
  const runBacktest = useCallback(async () => {
    ...
    try {
      ...
      const { results: backtestResults, summary: backtestSummary } =
        simulateBacktest(monthlyPrices, monthlyAmount, lastPrice);
