      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Taxa: {rate.fee}%</p>
          </div>
        </div>
        <Badge variant={isBest ? "success" : "outline"}>#{index + 1}</Badge>
      </div>

      <div className="grid grid-cols-3 gap-4 text-sm">
        <RateValue label="Cotação" value={rate.rate} />
        <RateValue label="Valor Bruto" value={rate.rate * amountNum} />
        <RateValue
          label="Valor Líquido"
