...
export function useBackupDiff(open: boolean) {
  ...
  const compareArrays = (arr1: unknown[], arr2: unknown[]): DiffResult => {
    const map1 = new Map(
      (arr1 as Record<string, unknown>[]).map((item) => [item.id, item]),
    );
    const map2 = new Map(
      (arr2 as Record<string, unknown>[]).map((item) => [item.id, item]),
    );

    const added = (arr2 as Record<string, unknown>[]).filter(
      (item) => !map1.has(item.id),
    );
    const removed = (arr1 as Record<string, unknown>[]).filter(
      (item) => !map2.has(item.id),
    );
  ...
  const compareMutation = async () => {
    ...
    try {
      const data1 = await apiClient.post<unknown>("/backups/manager", {
        backup_id: backup1,
      });
      const data2 = await apiClient.post<unknown>("/backups/manager", {
        backup_id: backup2,
      });
