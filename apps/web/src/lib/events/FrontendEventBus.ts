...
export class FrontendEventBus {
  ...
  private attemptReconnect(): void {
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      logger.error(
        "[FrontendEventBus] Máximo de tentativas de reconexão atingido",
      );
      return;
    }
    ...
    logger.info(
      `[FrontendEventBus] Reconectando em ${delay}ms (tentativa ${this.reconnectAttempts})`,
    );
  ...
  async publish(event: FrontendEvent): Promise<void> {
    ...
    await Promise.allSettled(
      handlers.map(async (handler) => {
        try {
          ...
          console.error(
            `[FrontendEventBus] Erro ao processar evento ${event.eventType}:`,
            error,
          );
        }
      }),
    ...
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      try {
        ...
        console.error(
          "[FrontendEventBus] Erro ao enviar evento via WebSocket:",
          error,
        );
      }
    }
