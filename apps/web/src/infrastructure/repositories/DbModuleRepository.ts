...
interface ModuleDependencyRow {
  dep_module?: { module_key: string; name: string };
}
interface ModuleDependentRow {
  module_catalog?: { module_key: string; name: string };
}
...
export class DbModuleRepository implements IModuleRepository {
  ...
  async findByClinicId(clinicId: string): Promise<Module[]> {
    try {
      ...
      return (data || []).map((row) => ModuleMapper.toDomain(row));
    } catch (error) {
      if (error instanceof InfrastructureError) throw error;
      throw new InfrastructureError(
  ...
  async activate(moduleId: number, clinicId: string): Promise<void> {
    try {
      await apiClient.post(`/configuracoes/modulos/${moduleId}/toggle`, {
        is_active: true,
        clinic_id: clinicId,
      });
    } catch (error) {
