...
export const useDatabaseAdmin = () => {
  ...
  const { data: slowQueriesData, isLoading: isLoadingSlowQueries } = useQuery({
    ...
    queryFn: async () => {
      return await apiClient.get<{ slowQueries: SlowQuery[] }>(
        "/database-admin/slow-queries",
      );
    },
  });
  ...
  const maintenanceMutation = useMutation({
    mutationFn: async (data: {
      operation: "VACUUM" | "ANALYZE" | "REINDEX" | "VACUUM_FULL";
      targetSchema?: string;
    }) => {
  ...
  const { data: poolData } = useQuery({
    ...
    queryFn: async () => {
      return await apiClient.get<{ poolStats: unknown }>(
        "/database-admin/connection-pool",
      );
    },
  });
