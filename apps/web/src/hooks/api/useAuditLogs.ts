...
export const useAuditLogs = (clinicId: string | undefined, isAdmin: boolean) => {
  ...
  const { data: logs = [], isLoading } = useQuery<AuditLog[]>({
    queryKey: ["audit-logs", clinicId, selectedUser, selectedAction, dateFrom, dateTo],
    queryFn: async () => {
      if (!clinicId) return []
      ...
      const data = await apiClient.get<AuditLog[]>(`/db/audit_logs?${params.toString()}`)
      return Array.isArray(data) ? data : []
    },
    enabled: isAdmin && !!clinicId,
  })
