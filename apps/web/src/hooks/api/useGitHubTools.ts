...
export const useGitHubTools = (repoName?: string) => {
  ...
  const { data: branches = [], isLoading: isLoadingBranches } = useQuery({
    ...
    queryFn: async () => {
      ...
      return await apiClient.get<Branch[]>(
        `/github-tools/repos/${repoName}/branches`,
      );
    },
    enabled: !!repoName,
  });
  ...
  const { data: pullRequests = [], isLoading: isLoadingPRs } = useQuery({
    ...
    queryFn: async () => {
      if (!repoName) return [];
      return await apiClient.get<PullRequest[]>(
        `/github-tools/repos/${repoName}/pulls`,
      );
    },
    enabled: !!repoName,
  });
  ...
  const executarWorkflow = useMutation({
    mutationFn: async (data: {
      ...
      return await apiClient.post(
        `/github-tools/repos/${data.repo_name}/workflows/${data.workflow_id}/dispatches`,
        {
          ref: data.ref,
        },
      );
