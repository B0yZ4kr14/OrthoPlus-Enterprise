...
export const useTerminal = () => {
  ...
  const { data: history = [], isLoading: isLoadingHistory } = useQuery({
    ...
    queryFn: async () => {
      ...
      return await apiClient.get<CommandResult[]>(
        `/terminal/sessions/${session.id}/history`,
      );
    },
    enabled: !!session?.id,
  });
  ...
  const executarComando = useMutation({
    mutationFn: async (data: { session_id: string; command: string }) => {
      return await apiClient.post<CommandResult>(
        `/terminal/sessions/${data.session_id}/execute`,
        {
