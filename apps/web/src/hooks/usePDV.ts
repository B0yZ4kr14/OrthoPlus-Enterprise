...
export const usePDV = (clinicId: string | undefined) => {
  ...
  const abrirCaixa = async (valorInicial: number, observacoes?: string) => {
    ...
    try {
      const data = await apiClient.post<CaixaMovimento>("/pdv/caixa/abrir", {
  ...
  const fecharCaixa = async (valorFinal: number, observacoes?: string) => {
    ...
    try {
      await apiClient.post(`/pdv/caixa/${caixaAberto.id}/fechar`, {
  ...
  const criarVenda = async (
    venda: Partial<PDVVenda>,
    itens: Partial<PDVVendaItem>[],
    pagamentos: Partial<PDVPagamento>[],
  ) => {
  ...
  const cancelarVenda = async (vendaId: string, motivo: string) => {
    ...
    try {
      await apiClient.post(`/pdv/vendas/${vendaId}/cancelar`, {
