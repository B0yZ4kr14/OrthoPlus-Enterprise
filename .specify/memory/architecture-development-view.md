...
| Backend API | Execute business logic; enforce invariants; persist data; produce events | HTTP requests in; JSON responses and events out | Database; Redis; Event Bus; External systems (email, SMS, S3) | UI rendering; frontend state management | Logical view: all capability boundaries; Process view: all runtime links |
